...
PathLoaderNode::PathLoaderNode() : nh_(), pnh_("~")
{
    pub_ = nh_.advertise<autoware_planning_msgs::Path>("path", 1, true);
...
bool PathLoaderNode::publish(const std_msgs::Header &header, const association &label_row_association_map, const csv &file_data)
{
...
bool PathLoaderNode::loadData(const std::string &file_name, association &label_row_association_map, csv &file_data)
{
    file_data.clear();
