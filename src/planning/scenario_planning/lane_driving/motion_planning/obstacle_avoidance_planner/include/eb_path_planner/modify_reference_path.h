...
class ModifyReferencePath {
 private:
  ...
  bool expandNode(Node& parent_node, const cv::Mat& clearence_map, const nav_msgs::MapMetaData& map_info,
                  const Node& goal_node, const double min_r, const double max_r, std::vector<Node>& expanded_nodes);
  bool expandGridNode(const GridNode& parent_node, const cv::Mat& clearance_map, const nav_msgs::MapMetaData& map_info,
                      const GridNode& goal_node, cv::Mat& visited_map, std::vector<GridNode>& child_nodes);
  bool isOverlap(Node& node1, Node& node2);
  bool nodeExistInClosedNodes(Node node, std::vector<Node> closed_nodes);
  bool solveGraphAStar(const geometry_msgs::Pose& ego_pose, const geometry_msgs::Point& start_point_in_map,
                       const geometry_msgs::Point& goal_point_in_map, const cv::Mat& clearance_map,
                       const nav_msgs::MapMetaData& map_info, std::vector<geometry_msgs::Point>& explored_points);
  bool solveAStar(const geometry_msgs::Point& start_point_in_map, const geometry_msgs::Point& goal_point_in_map,
                  const cv::Mat& clearance_map, const nav_msgs::MapMetaData& map_info,
                  std::vector<geometry_msgs::Point>& explored_points);

  bool arrangeExploredPointsBaseedOnClearance(const cv::Mat& clearance_map, const geometry_msgs::Pose& ego_pose,
                                              std::vector<geometry_msgs::Point>& explored_points,
                                              std::vector<geometry_msgs::Point>& debug_rearranged_points);

 public:
  bool generateModifiedPath(const geometry_msgs::Pose& ego_pose, const geometry_msgs::Point& start_exploring_point,
                            ...
                            std::vector<geometry_msgs::Point>& explored_points, const cv::Mat& clearance_map,
                            const nav_msgs::MapMetaData& map_info);
  ModifyReferencePath(double min_radius, double backward_distance, double clearance_weight_when_exploring);
  ModifyReferencePath();
  ~ModifyReferencePath();
};
