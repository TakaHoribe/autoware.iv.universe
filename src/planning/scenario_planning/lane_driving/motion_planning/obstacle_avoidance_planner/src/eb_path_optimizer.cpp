...
EBPathOptimizer::EBPathOptimizer(const bool is_showing_debug_info, const QPParam qp_param,
                                 const TrajectoryParam traj_pram, const ConstrainParam constrain_param)
...
void EBPathOptimizer::initializeSolver() {
  ...
  osqp_solver_ptr_ =
      std::make_unique<osqp::OSQPInterface>(p, default_a_matrix_, q, lower_bound, upper_bound, qp_param_.eps_abs);
...
Eigen::MatrixXd EBPathOptimizer::makePMatrix() {
  Eigen::MatrixXd P = Eigen::MatrixXd::Zero(traj_param_.num_sampling_points * 2, traj_param_.num_sampling_points * 2);
...
Eigen::MatrixXd EBPathOptimizer::makeAMatrix() {
  Eigen::MatrixXd A =
      Eigen::MatrixXd::Identity(traj_param_.num_sampling_points * 2, traj_param_.num_sampling_points * 2);
...
std::vector<autoware_planning_msgs::TrajectoryPoint> EBPathOptimizer::generateOptimizedTrajectory(
    const bool enable_avoidance, const geometry_msgs::Pose& ego_pose, const autoware_planning_msgs::Path& path,
    const std::unique_ptr<std::vector<autoware_planning_msgs::TrajectoryPoint>>& prev_optimized_points,
  ...
  CandidatePoints candidate_points =
      getCandidatePoints(ego_pose, path.points, prev_optimized_points, drivable_area, path.drivable_area.info);
  ...
  if (candidate_points.fixed_points.empty() && candidate_points.non_fixed_points.empty()) {
    std::vector<autoware_planning_msgs::TrajectoryPoint> traj_points = util::convertPathToTrajectory(path.points);
    return traj_points;
  }

  // get optimized smooth points
  std::vector<autoware_planning_msgs::TrajectoryPoint> traj_points = getOptimizedTrajectory(
      enable_avoidance, path, candidate_points, clearance_map, only_objects_clearance_map, debug_data);
...
std::vector<autoware_planning_msgs::TrajectoryPoint> EBPathOptimizer::getOptimizedTrajectory(
    const bool enable_avoidance, const autoware_planning_msgs::Path& path, const CandidatePoints& candidate_points,
    const cv::Mat& clearance_map, const cv::Mat& only_objects_clearance_map, DebugData& debug_data) {
  ...
  const int farrest_idx = std::min((int)(traj_param_.num_sampling_points - 1), (int)(interpolated_points.size() - 1));
  const int num_fixed_points = getNumFixiedPoints(candidate_points.fixed_points, interpolated_points, farrest_idx);
  const int straight_line_idx = getStraightLineIdx(interpolated_points, farrest_idx, only_objects_clearance_map,
                                                   path.drivable_area.info, debug_data.straight_points);
  std::vector<geometry_msgs::Point> padded_interpolated_points =
      getPaddedInterpolatedPoints(interpolated_points, farrest_idx);
  auto t_start1 = std::chrono::high_resolution_clock::now();
  std::vector<ConstrainRectangle> rectangles =
      getConstrainRectangleVec(enable_avoidance, path, padded_interpolated_points, num_fixed_points, farrest_idx,
...
std::vector<autoware_planning_msgs::TrajectoryPoint> EBPathOptimizer::getExtendedOptimizedTrajectory(
  ...
  if (accum_arc_length > traj_param_.trajectory_length || candidate_points.end_path_idx == path_points.size() ||
  ...
  const double delat_arc_length_for_extending_trajectory =
  ...
  std::vector<geometry_msgs::Point> interpolated_points =
      util::getInterpolatedPoints(fixed_poitns, non_fixed_points, delat_arc_length_for_extending_trajectory);
  const int farrest_idx = std::min((int)(traj_param_.num_sampling_points - 1), (int)(interpolated_points.size() - 1));
  std::vector<geometry_msgs::Point> padded_interpolated_points =
      getPaddedInterpolatedPoints(interpolated_points, farrest_idx);
  ...
  const auto extended_traj_points = convertOptimizedPointsToTrajectory(extended_optimized_points, farrest_idx);
...
std::vector<geometry_msgs::Pose> EBPathOptimizer::getFixedPoints(
  ...
  if (prev_optimized_points) {
    ...
    int begin_idx = util::getNearestIdx(*prev_optimized_points, ego_pose, default_idx,
                                        traj_param_.delta_yaw_threshold_for_closest_point);
    if (!isPointInsideDrivableArea(prev_optimized_points->at(begin_idx).pose.position, drivable_area, map_info)) {
      begin_idx = default_idx;
    }
    const int backward_fixing_idx = std::max(
        (int)(begin_idx - traj_param_.backward_fixing_distance / traj_param_.delta_arc_length_for_trajectory), 0);
    const int forward_fixing_idx =
        std::min((int)(begin_idx + traj_param_.forward_fixing_distance / traj_param_.delta_arc_length_for_trajectory),
...
CandidatePoints EBPathOptimizer::getCandidatePoints(
  ...
  int begin_idx = util::getNearestIdx(path_points, fixed_points.back(), default_idx,
                                      traj_param_.delta_yaw_threshold_for_closest_point);
  ...
  begin_idx = std::min((int)begin_idx + traj_param_.num_offset_for_begin_idx, (int)path_points.size());
...
CandidatePoints EBPathOptimizer::getDefaultCandidatePoints(
  ...
  for (size_t i = 0; i < path_points.size(); i++) {
    if (i > 0) {
      accum_arc_length += util::calculate2DDistance(path_points[i].pose.position, path_points[i - 1].pose.position);
    }
    if (accum_arc_length > traj_param_.num_sampling_points * traj_param_.delta_arc_length_for_optimization) {
      break;
    }
    end_path_idx = i;
...
bool EBPathOptimizer::isPointInsideDrivableArea(const geometry_msgs::Point& point, const cv::Mat& drivable_area,
...
int EBPathOptimizer::getEndPathIdx(const std::vector<autoware_planning_msgs::PathPoint>& path_points,
  ...
  for (int i = begin_path_idx; i < path_points.size(); i++) {
    if (i > begin_path_idx) {
      const double dist = util::calculate2DDistance(path_points[i].pose.position, path_points[i - 1].pose.position);
      accum_dist += dist;
    }
    end_path_idx = i;
...
int EBPathOptimizer::getEndPathIdxInsideArea(const std::vector<autoware_planning_msgs::PathPoint>& path_points,
                                             const int begin_path_idx, const int end_path_idx,
                                             const cv::Mat& drivable_area, const nav_msgs::MapMetaData& map_info) {
  ...
  for (int i = begin_path_idx; i <= end_path_idx; i++) {
    ...
    rel_top_point.y = 0;
    geometry_msgs::Point abs_top_point = util::transformToAbsoluteCoordinate2D(rel_top_point, path_points[i].pose);
...
double EBPathOptimizer::getArcLength(const std::vector<autoware_planning_msgs::TrajectoryPoint>& optimized_points) {
  double accum_arc_length = 0;
  for (size_t i = 0; i < optimized_points.size(); i++) {
    if (i > 0) {
      const double dist =
          util::calculate2DDistance(optimized_points[i].pose.position, optimized_points[i - 1].pose.position);
...
int EBPathOptimizer::getNumFixiedPoints(const std::vector<geometry_msgs::Pose>& fixed_points,
...
std::vector<autoware_planning_msgs::TrajectoryPoint> EBPathOptimizer::convertOptimizedPointsToTrajectory(
  ...
  for (size_t i = 0; i < traj_points.size(); i++) {
    if (i > 0) {
      traj_points[i].pose.orientation =
          util::getQuaternionFromPoints(traj_points[i].pose.position, traj_points[i - 1].pose.position);
    } else if (i == 0 && traj_points.size() > 1) {
      traj_points[i].pose.orientation =
          util::getQuaternionFromPoints(traj_points[i + 1].pose.position, traj_points[i].pose.position);
    }
  }
  return traj_points;
}
...
std::vector<ConstrainRectangle> EBPathOptimizer::getConstrainRectangleVec(
  ...
  for (int i = 0; i < traj_param_.num_sampling_points; i++) {
    ...
    if (i == 0 || i == 1 || i >= farrest_point_idx - 1 || i < num_fixed_points - 1) {
      ConstrainRectangle rectangle = getConstrainRectangle(origin_pose, constrain_param_.clearance_for_fixing);
      object_road_constrain_ranges[i] = rectangle;
      road_constrain_ranges[i] = rectangle;
      only_smooth_constrain_ranges[i] = rectangle;
    } else if (i >= num_fixed_points - traj_param_.num_joint_buffer_points &&
      ...
      ConstrainRectangle rectangle = getConstrainRectangle(origin_pose, constrain_param_.clearance_for_straight_line);
      object_road_constrain_ranges[i] = rectangle;
      road_constrain_ranges[i] = rectangle;
      only_smooth_constrain_ranges[i] = rectangle;
    } else {
      ConstrainRectangles constrain_rectangles =
          getConstrainRectangles(origin_pose, clearance_map, only_objects_clearance_map, path.drivable_area.info);
  ...
  std::vector<ConstrainRectangle> constrain_ranges = getPostProcessedConstrainRectangles(
      enable_avoidance, object_road_constrain_ranges, road_constrain_ranges, only_smooth_constrain_ranges,
      interpolated_points, path.points, farrest_point_idx, num_fixed_points, straight_idx);
...
std::vector<ConstrainRectangle> EBPathOptimizer::getConstrainRectangleVec(
  ...
  for (int i = 0; i < traj_param_.num_sampling_points; i++) {
    ...
    if (i == 0 || i == 1 || i >= farrest_point_idx - 1) {
      ...
      only_smooth_constrain_ranges[i] = rectangle;
    } else if (i >= num_fixed_points, i <= num_fixed_points + traj_param_.num_fix_points_for_extending) {
      ConstrainRectangle rectangle = getConstrainRectangle(origin_pose, constrain_param_.clearance_for_joint);
      only_smooth_constrain_ranges[i] = rectangle;
    } else {
      ConstrainRectangle rectangle = getConstrainRectangle(origin_pose, constrain_param_.clearance_for_only_smoothing);
      only_smooth_constrain_ranges[i] = rectangle;
    }
  }
...
std::vector<ConstrainRectangle> EBPathOptimizer::getPostProcessedConstrainRectangles(
  ...
  for (size_t i = 0; i < traj_param_.num_sampling_points; i++) {
    if (i == 0 || i == 1 || i >= farrest_point_idx - 1 || i < num_fixed_points - 1 ||
      ...
      if (i > origin_dynamic_joint_idx && i <= origin_dynamic_joint_idx + traj_param_.num_joint_buffer_points) {
        geometry_msgs::Pose origin_pose = getOriginPose(interpolated_points, i, path_points);
        ConstrainRectangle rectangle = getConstrainRectangle(origin_pose, constrain_param_.clearance_for_joint);
        ...
        if (constrain_ranges[i].is_dynamic_joint_rectangle) {
          ROS_INFO_COND(is_showing_debug_info_, "Only road clearance optimization failed at %zu", i);
          is_using_only_smooth_constrain = true;
...
void EBPathOptimizer::updateConstrain(const std::vector<geometry_msgs::Point>& interpolated_points,
                                      const std::vector<ConstrainRectangle>& rectangle_points) {
  Eigen::MatrixXd A = default_a_matrix_;
  std::vector<double> lower_bound(traj_param_.num_sampling_points * 2, 0.0);
  std::vector<double> upper_bound(traj_param_.num_sampling_points * 2, 0.0);
  for (int i = 0; i < traj_param_.num_sampling_points; ++i) {
    Constrain constrain = getConstrainFromConstrainRectangle(interpolated_points[i], rectangle_points[i]);
    ...
    A(i + traj_param_.num_sampling_points, i + traj_param_.num_sampling_points) = constrain.left_and_right.y_coef;
...
Rectangle EBPathOptimizer::getAbsShapeRectangle(const Rectangle& rel_shape_rectangle_points,
                                                const geometry_msgs::Point& offset_point,
                                                const geometry_msgs::Pose& origin) {
  geometry_msgs::Point abs_target_point = util::transformToAbsoluteCoordinate2D(offset_point, origin);
...
geometry_msgs::Pose EBPathOptimizer::getOriginPose(const std::vector<geometry_msgs::Point>& interpolated_points,
                                                   const int interpolated_idx,
                                                   const std::vector<autoware_planning_msgs::PathPoint>& path_points) {
  ...
  if (interpolated_idx > 0) {
    pose.orientation =
        util::getQuaternionFromPoints(interpolated_points[interpolated_idx], interpolated_points[interpolated_idx - 1]);
  } else if (interpolated_idx == 0 && interpolated_points.size() > 1) {
    pose.orientation =
        util::getQuaternionFromPoints(interpolated_points[interpolated_idx + 1], interpolated_points[interpolated_idx]);
  }
  const int default_idx = 0;
  const int nearest_id =
      util::getNearestIdx(path_points, pose, default_idx, traj_param_.delta_yaw_threshold_for_closest_point);
...
std::shared_ptr<std::vector<std::vector<geometry_msgs::Point>>> EBPathOptimizer::getOccupancyPoints(
    const geometry_msgs::Pose& origin, const cv::Mat& clearance_map, const nav_msgs::MapMetaData& map_info) {
  ...
  const float clearance =
      clearance_map.ptr<float>((int)interpolated_image_point_ptr->y)[(int)interpolated_image_point_ptr->x] *
      map_info.resolution;
  ...
  std::vector<std::vector<geometry_msgs::Point>> occupancy_points(x_side_length,
                                                                  std::vector<geometry_msgs::Point>(y_side_length));
  ...
  for (float x = -1 * x_constrain_search_range; x <= x_constrain_search_range + epsilon_;
    ...
    for (float y = -1 * y_constrain_search_range; y <= y_constrain_search_range + epsilon_;
      ...
      relative_point.x = x;
      relative_point.y = y;
      geometry_msgs::Point abs_point = util::transformToAbsoluteCoordinate2D(relative_point, origin);
      occupancy_points[x_side_length - x_idx_in_occupancy_map - 1][y_side_length - y_idx_in_occupancy_map - 1] =
          abs_point;
  ...
  occupancy_points_ptr = std::make_shared<std::vector<std::vector<geometry_msgs::Point>>>(occupancy_points);
  return occupancy_points_ptr;
}

Rectangle EBPathOptimizer::getRelShapeRectangle(const geometry_msgs::Vector3& vehicle_shape,
                                                const geometry_msgs::Pose& origin) {
...
ConstrainRectangles EBPathOptimizer::getConstrainRectangles(const geometry_msgs::Pose& origin_pose,
                                                            const cv::Mat& clearance_map,
                                                            const cv::Mat& only_objects_clearance_map,
                                                            const nav_msgs::MapMetaData& map_info) {
  const auto occupancy_points_ptr = getOccupancyPoints(origin_pose, clearance_map, map_info);

  std::shared_ptr<geometry_msgs::Point> image_point_ptr = util::transformMapToImagePtr(origin_pose.position, map_info);
  ...
  OccupancyMaps occupancy_maps = getOccupancyMaps(*occupancy_points_ptr, origin_pose, *image_point_ptr, clearance_map,
...
OccupancyMaps EBPathOptimizer::getOccupancyMaps(const std::vector<std::vector<geometry_msgs::Point>>& occupancy_points,
                                                const geometry_msgs::Pose& origin_pose,
                                                const geometry_msgs::Point& origin_point_in_image,
  ...
  const float clearance =
      clearance_map.ptr<float>((int)origin_point_in_image.y)[(int)origin_point_in_image.x] * map_info.resolution;
  const float y_constrain_search_range = clearance - 0.5 * keep_space_shape_ptr_->y;
  const float x_constrain_search_range =
      std::fmin(constrain_param_.max_x_constrain_search_range, y_constrain_search_range);
  std::vector<std::vector<int>> object_occupancy_map(occupancy_points.size(),
                                                     std::vector<int>(occupancy_points.front().size(), 0));
  std::vector<std::vector<int>> road_occupancy_map(occupancy_points.size(),
                                                   std::vector<int>(occupancy_points.front().size(), 0));
  int x_idx_in_occupancy_map = 0;
  int y_idx_in_occupancy_map = 0;
  for (float x = -1 * x_constrain_search_range; x <= x_constrain_search_range + epsilon_;
    ...
    for (float y = -1 * y_constrain_search_range; y <= y_constrain_search_range + epsilon_;
      ...
      Rectangle abs_shape_rectangles = getAbsShapeRectangle(rel_shape_rectangles, rel_target_point, origin_pose);
      ...
      if (util::transformMapToImage(abs_shape_rectangles.top_left, map_info, top_left_image)) {
        top_left_clearance =
            clearance_map.ptr<float>((int)top_left_image.y)[(int)top_left_image.x] * map_info.resolution;
        top_left_objects_clearance =
            only_objects_clearance_map.ptr<float>((int)top_left_image.y)[(int)top_left_image.x] * map_info.resolution;
      }
      ...
      if (util::transformMapToImage(abs_shape_rectangles.top_right, map_info, top_right_image)) {
        top_right_clearance =
            clearance_map.ptr<float>((int)top_right_image.y)[(int)top_right_image.x] * map_info.resolution;
        top_right_objects_clearance =
            only_objects_clearance_map.ptr<float>((int)top_right_image.y)[(int)top_right_image.x] * map_info.resolution;
      }
      ...
      if (util::transformMapToImage(abs_shape_rectangles.bottom_left, map_info, bottom_left_image)) {
        bottom_left_clearance =
            clearance_map.ptr<float>((int)bottom_left_image.y)[(int)bottom_left_image.x] * map_info.resolution;
        bottom_left_objects_clearance =
            only_objects_clearance_map.ptr<float>((int)bottom_left_image.y)[(int)bottom_left_image.x] *
            map_info.resolution;
      }
      ...
      if (util::transformMapToImage(abs_shape_rectangles.bottom_right, map_info, bottom_right_image)) {
        bottom_right_clearance =
            clearance_map.ptr<float>((int)bottom_right_image.y)[(int)bottom_right_image.x] * map_info.resolution;
        bottom_right_objects_clearance =
            only_objects_clearance_map.ptr<float>((int)bottom_right_image.y)[(int)bottom_right_image.x] *
            map_info.resolution;
      }
      if (top_left_clearance < constrain_param_.min_clearance_from_road ||
...
int EBPathOptimizer::getStraightLineIdx(const std::vector<geometry_msgs::Point>& interpolated_points,
                                        const int farrest_point_idx, const cv::Mat& only_objects_clearance_map,
                                        const nav_msgs::MapMetaData& map_info,
...
Constrain EBPathOptimizer::getConstrainFromConstrainRectangle(const geometry_msgs::Point& interpolated_point,
  ...
  if (std::fabs(top_dx) < epsilon_ && std::fabs(top_dy) < epsilon_ && std::fabs(left_dx) < epsilon_ &&
    ...
    constrain.left_and_right = getConstrainLines(left_dx, left_dy, constrain_range.top_left, constrain_range.top_right);
    ...
    constrain.top_and_bottom = getConstrainLines(top_dx, top_dy, constrain_range.top_left, constrain_range.bottom_left);
...
ConstrainLines EBPathOptimizer::getConstrainLines(const double dx, const double dy, const geometry_msgs::Point& point,
...
ConstrainRectangle EBPathOptimizer::getConstrainRectangle(const geometry_msgs::Pose& origin_pose,
...
ConstrainRectangle EBPathOptimizer::getConstrainRectangle(
    const std::vector<autoware_planning_msgs::PathPoint>& path_points, const geometry_msgs::Pose& origin_pose,
  ...
  const int nearest_idx = util::getNearestIdx(interpolated_points, origin_pose, default_idx,
                                              traj_param_.delta_yaw_threshold_for_closest_point);
  ...
  if (util::transformMapToImage(interpolated_points[nearest_idx], map_info, image_point)) {
    clearance = clearance_map.ptr<float>((int)image_point.y)[(int)image_point.x] * map_info.resolution;
  }
  const double dist = util::calculate2DDistance(origin_pose.position, interpolated_points[nearest_idx]);
  ...
  if (nearest_idx > default_idx && dist < clearance) {
    ...
    if (nearest_idx > 0) {
      replaced_pose.orientation =
          util::getQuaternionFromPoints(interpolated_points[nearest_idx], interpolated_points[nearest_idx - 1]);
    } else if (nearest_idx == 0 && interpolated_points.size() > 1) {
      replaced_pose.orientation =
          util::getQuaternionFromPoints(interpolated_points[nearest_idx + 1], interpolated_points[nearest_idx]);
    }
    ConstrainRectangles rectangles = getConstrainRectangles(replaced_pose, clearance_map, clearance_map, map_info);
    const double rel_plus_y =
        util::calculate2DDistance(rectangles.road_constrain_rectangle.top_left, replaced_pose.position);
    const double rel_minus_y =
        util::calculate2DDistance(rectangles.road_constrain_rectangle.top_right, replaced_pose.position);
    geometry_msgs::Point top_left;
    ...
    constrain_rectangle.bottom_left = util::transformToAbsoluteCoordinate2D(bottom_left, replaced_pose);
    geometry_msgs::Point bottom_right;
    bottom_right.x = -1 * constrain_param_.clearance_for_joint;
    bottom_right.y = -1 * rel_minus_y;
    constrain_rectangle.bottom_right = util::transformToAbsoluteCoordinate2D(bottom_right, replaced_pose);
  } else {
...
ConstrainRectangle EBPathOptimizer::getConstrainRectangle(
    ...
    const std::vector<std::vector<geometry_msgs::Point>>& occupancy_points, const geometry_msgs::Pose& origin_pose) {
  util::Rectangle util_rect = util::getLargestRectangle(occupancy_map);
  double lat_space = util::calculate2DDistance(occupancy_points[util_rect.max_x_idx][util_rect.max_y_idx],
                                               occupancy_points[util_rect.max_x_idx][util_rect.min_y_idx]);
  double lon_space = util::calculate2DDistance(occupancy_points[util_rect.max_x_idx][util_rect.max_y_idx],
                                               occupancy_points[util_rect.min_x_idx][util_rect.max_y_idx]);
  if (lat_space <= constrain_param_.min_lat_constrain_space + epsilon_ ||
    ...
    lat_space = util::calculate2DDistance(occupancy_points[util_rect.max_x_idx][util_rect.max_y_idx],
                                          occupancy_points[util_rect.max_x_idx][util_rect.min_y_idx]);
    lon_space = util::calculate2DDistance(occupancy_points[util_rect.max_x_idx][util_rect.max_y_idx],
                                          occupancy_points[util_rect.min_x_idx][util_rect.max_y_idx]);
  }

  ConstrainRectangle constrain_rectangle;
  if (util_rect.area < epsilon_ || lat_space <= constrain_param_.min_lat_constrain_space + epsilon_ ||
      lon_space <= constrain_param_.min_lon_constrain_space + epsilon_) {
    constrain_rectangle = getConstrainRectangle(origin_pose, constrain_param_.clearance_for_joint);
...
bool EBPathOptimizer::isFixingPathPoint(const std::vector<autoware_planning_msgs::PathPoint>& path_points) {
