...
ObstacleAvoidancePlanner::ObstacleAvoidancePlanner() : min_num_points_for_getting_yaw_(2), nh_(), pnh_("~") {
  ...
  debug_clearance_map_pub_ = pnh_.advertise<nav_msgs::OccupancyGrid>("debug/clearance_map", 1, true);
  debug_object_clearance_map_pub_ = pnh_.advertise<nav_msgs::OccupancyGrid>("debug/object_clearance_map", 1, true);

  path_sub_ = pnh_.subscribe("input/path", 1, &ObstacleAvoidancePlanner::pathCallback, this);
  objects_sub_ = pnh_.subscribe("input/objects", 10, &ObstacleAvoidancePlanner::objectsCallback, this);
  state_sub_ = pnh_.subscribe("/autoware/state", 10, &ObstacleAvoidancePlanner::stateCallback, this);
  is_avoidance_sub_ = pnh_.subscribe("enable_avoidance", 10, &ObstacleAvoidancePlanner::enableAvoidanceCallback, this);
  ...
  pnh_.param<double>("delta_arc_length_for_optimization", traj_param_->delta_arc_length_for_optimization, 1.0);
  pnh_.param<double>("delta_arc_length_for_trajectory", traj_param_->delta_arc_length_for_trajectory, 0.1);
  pnh_.param<double>("delta_yaw_threshold_for_closest_point", traj_param_->delta_yaw_threshold_for_closest_point, 1.0);
  pnh_.param<double>("delta_yaw_threshold_for_straight", traj_param_->delta_yaw_threshold_for_straight, 0.02);
  ...
  pnh_.param<double>("clearance_for_straight_line_", constrain_param_->clearance_for_straight_line, 0.05);
  pnh_.param<double>("clearance_for_joint_", constrain_param_->clearance_for_joint, 3.2);
  pnh_.param<double>("clearance_for_only_smoothing", constrain_param_->clearance_for_only_smoothing, 0.1);
  pnh_.param<double>("clearance_from_object_for_straight", constrain_param_->clearance_from_object_for_straight, 10.0);
  ...
  pnh_.param<double>("max_x_constrain_search_range", constrain_param_->max_x_constrain_search_range, 0.4);
  pnh_.param<double>("max_avoiding_objects_velocity_ms", constrain_param_->max_avoiding_objects_velocity_ms, 0.1);
  pnh_.param<double>("coef_x_cosntrain_search_resolution", constrain_param_->coef_x_cosntrain_search_resolution, 1.0);
  pnh_.param<double>("coef_y_cosntrain_search_resolution", constrain_param_->coef_y_cosntrain_search_resolution, 0.5);
  ...
  pnh_.param<double>("distance_for_path_shape_chagne_detection", distance_for_path_shape_chagne_detection_, 2.0);
...
void ObstacleAvoidancePlanner::objectsCallback(const autoware_perception_msgs::DynamicObjectArray& msg) {
...
void ObstacleAvoidancePlanner::enableAvoidanceCallback(const std_msgs::Bool& msg) { enable_avoidance_ = msg.data; }
...
autoware_planning_msgs::Trajectory ObstacleAvoidancePlanner::generateTrajectory(
  ...
  const auto post_processed_traj = generatePostProcessedTrajectory(*current_ego_pose_ptr_, path.points, traj_points);

  autoware_planning_msgs::Trajectory output;
  output.header = path.header;
  output.points = post_processed_traj;

  prev_traj_points_ptr_ = std::make_unique<std::vector<autoware_planning_msgs::TrajectoryPoint>>(output.points);
  prev_path_points_ptr_ = std::make_unique<std::vector<autoware_planning_msgs::PathPoint>>(path.points);

  auto t_end = std::chrono::high_resolution_clock::now();
  float elapsed_ms = std::chrono::duration<float, std::milli>(t_end - t_start).count();
  ROS_INFO_COND(is_showing_debug_info_, "Total time: = %f [ms]\n==========================", elapsed_ms);
  return output;
}

std::vector<autoware_planning_msgs::TrajectoryPoint> ObstacleAvoidancePlanner::generateOptimizedTrajectory(
    const geometry_msgs::Pose& ego_pose, const autoware_planning_msgs::Path& path) {
  if (!needReplan(ego_pose, prev_ego_pose_ptr_, path.points, prev_replanned_time_ptr_, prev_path_points_ptr_,
  ...
  debug_markers_pub_.publish(getDebugVisualizationMarker(debug_data.interpolated_points, trajectory_points,
                                                         debug_data.straight_points, debug_data.fixed_points,
                                                         debug_data.non_fixed_points, debug_data.constrain_rectangles));
  if (is_publishing_clearance_map_) {
    ...
    debug_object_clearance_map_pub_.publish(getDebugCostmap(debug_data.only_object_clearance_map, path.drivable_area));
  }
...
void ObstacleAvoidancePlanner::initialize() {
  ...
  eb_path_optimizer_ptr_ =
      std::make_unique<EBPathOptimizer>(is_showing_debug_info_, *qp_param_, *traj_param_, *constrain_param_);
...
std::vector<autoware_planning_msgs::TrajectoryPoint> ObstacleAvoidancePlanner::generatePostProcessedTrajectory(
...
bool ObstacleAvoidancePlanner::isPathShapeChanged(
  ...
  const int nearest_prev_path_idx = util::getNearestIdx(*prev_path_points, ego_pose, default_nearest_prev_path_idx,
                                                        traj_param_->delta_yaw_threshold_for_closest_point);
  const int default_nearest_path_idx = 0;
  const int nearest_path_idx = util::getNearestIdx(path_points, ego_pose, default_nearest_path_idx,
                                                   traj_param_->delta_yaw_threshold_for_closest_point);
...
std::vector<autoware_planning_msgs::TrajectoryPoint> ObstacleAvoidancePlanner::convertPointsToTrajectory(
    const std::vector<autoware_planning_msgs::PathPoint>& path_points,
  ...
  std::shared_ptr<geometry_msgs::Point> extended_point_ptr = getExtendedPoint(path_points, candidate_points);
  ...
  candidate_back.orientation = util::getQuaternionFromPoints(candidate_points[candidate_points.size() - 1],
                                                             candidate_points[candidate_points.size() - 2]);
  const int default_path_idx = path_points.size() - 1;
  const int nearest_path_idx_from_back = util::getNearestIdx(path_points, candidate_back, default_path_idx,
                                                             traj_param_->delta_yaw_threshold_for_closest_point);
  ...
  const int zero_velocity_traj_idx =
      util::getNearestIdx(candidate_points, path_points[nearest_path_idx].pose, default_idx,
                          traj_param_->delta_yaw_threshold_for_closest_point);
  const float debug_dist = util::calculate2DDistance(path_points[zero_velocity_path_points_idx].pose.position,
  ...
  for (int i = 0; i < traj_points.size(); i++) {
    std::vector<autoware_planning_msgs::PathPoint>::const_iterator first = path_points.begin() + prev_begin_path_idx;
    ...
    const int nearest_path_idx = util::getNearestIdx(truncated_path_points, traj_points[i].pose, default_idx);
