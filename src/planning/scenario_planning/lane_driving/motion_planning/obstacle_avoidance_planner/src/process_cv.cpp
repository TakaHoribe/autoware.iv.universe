...
void getOccupancyGridValue(const nav_msgs::OccupancyGrid& og, const int i, const int j, unsigned char& value) {
...
void putOccupancyGridValue(nav_msgs::OccupancyGrid& og, const int i, const int j, const unsigned char value) {
  int i_flip = og.info.width - i - 1;
  int j_flip = og.info.height - j - 1;
  og.data[i_flip + j_flip * og.info.width] = value;
}

cv::Mat drawObstalcesOnImage(const std::vector<autoware_perception_msgs::DynamicObject>& objects,
  ...
  for (const auto& object : objects) {
    ...
    const double vel =
        std::sqrt(object.state.twist_covariance.twist.linear.x * object.state.twist_covariance.twist.linear.x +
    ...
    if (vel < max_avoiding_objects_velocity_ms && clearance > 0) {
      if (object.shape.type == object.shape.BOUNDING_BOX) {
        ...
        cv::circle(objects_image, cv::Point(image_point_ptr->x, image_point_ptr->y),
...
std::vector<cv::Point> getCVPolygonFromBoundingBox(const autoware_perception_msgs::DynamicObject& object,
  ...
  geometry_msgs::Point top_right_map = util::transformToAbsoluteCoordinate2D(top_right, object_pose);

  geometry_msgs::Point bottom_left;
  bottom_left.x = -1 * object.shape.dimensions.x * 0.5;
  bottom_left.y = object.shape.dimensions.y * 0.5;
  geometry_msgs::Point bottom_left_map = util::transformToAbsoluteCoordinate2D(bottom_left, object_pose);
  ...
  geometry_msgs::Point bottom_right_map = util::transformToAbsoluteCoordinate2D(bottom_right, object_pose);
  ...
  geometry_msgs::Point bottom_left_map_in_image = util::transformMapToImage(bottom_left_map, map_info);
  geometry_msgs::Point bottom_right_map_in_image = util::transformMapToImage(bottom_right_map, map_info);
...
std::vector<cv::Point> getCVPolygonFromPolygon(const autoware_perception_msgs::DynamicObject& object,
                                               const nav_msgs::MapMetaData& map_info) {
  std::vector<cv::Point> cv_polygon;
...
cv::Mat getOnlyObjectsClearanceMap(const cv::Mat& clearance_map,
  ...
  cv::Mat objects_image =
      drawObstalcesOnImage(objects, occupancy_grid.info, clearance_map, max_avoiding_objects_velocity_ms);
...
cv::Mat getClearanceMap(const nav_msgs::OccupancyGrid& occupancy_grid, const cv::Mat& drivable_area,
                        const std::vector<autoware_perception_msgs::DynamicObject>& objects) {
