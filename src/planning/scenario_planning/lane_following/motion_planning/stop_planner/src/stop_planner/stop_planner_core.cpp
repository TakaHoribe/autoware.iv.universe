...
StopPlanner::StopPlanner() : nh_(""), pnh_("~"), tf_listener_(tf_buffer_) {
  ...
  pnh_.param<double>("/vehicle_info/wheel_tread", tread, 2.0);
  pnh_.param<double>("/vehicle_info/front_overhang", front_overhang, 1.5);
  pnh_.param<double>("/vehicle_info/rear_overhang", rear_overhang, 1.5);
  pnh_.param<double>("stop_margin_dist", stop_margin_dist, 7.0);
  pnh_.param<int>("object_pcd_threshold_num", object_pcd_threshold_num, 1);
  pnh_.param<double>("detection_area_width", detection_area_width, 1.5);
...
void StopPlanner::callbackTrajectory(const autoware_planning_msgs::Trajectory::ConstPtr& msg) {
  ...
  ROS_INFO_COND(false, "obstacle processing time : %lf [ms]", calctime_ms);
