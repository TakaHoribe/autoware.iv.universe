...
TEST_F(TestSuite, CalculateDistanceBetween2Points) {
  ...
  double x2 = 3.0;
  double y2 = 4.0;
  ASSERT_EQ(calcDistance(x1, y1, x2, y2), sqrt(x2 * x2 + y2 * y2))
      << "Distance should be " << sqrt(x2 * x2 + y2 * y2);
}
...
TEST_F(TestSuite, CheckTransformPose) {
  ...
  tf2::convert(v, transform.transform.translation);
  tf2::convert(q, transform.transform.rotation);

  out_pose = transformPose(in_pose, transform);

  ASSERT_DOUBLE_EQ(out_pose.position.x, expected_pose.position.x)
      << "X Coordinate should be " << expected_pose.position.x;
  ASSERT_DOUBLE_EQ(out_pose.position.y, expected_pose.position.y)
      << "Y Coordinate should be " << expected_pose.position.y;
  ASSERT_DOUBLE_EQ(out_pose.position.z, expected_pose.position.z)
      << "Z Coordinate should be " << expected_pose.position.z;
  ASSERT_DOUBLE_EQ(out_pose.orientation.x, expected_pose.orientation.x)
      << "X Quaternion should be " << expected_pose.orientation.x;
  ASSERT_DOUBLE_EQ(out_pose.orientation.y, expected_pose.orientation.y)
      << "Y Quaternion should be " << expected_pose.orientation.y;
  ASSERT_DOUBLE_EQ(out_pose.orientation.z, expected_pose.orientation.z)
      << "Z Quaternion should be " << expected_pose.orientation.z;
  ASSERT_DOUBLE_EQ(out_pose.orientation.w, expected_pose.orientation.w)
      << "W Quaternion should be " << expected_pose.orientation.w;
}
...
TEST_F(TestSuite, CheckRadianDifference) {
  ...
  a = 0 * M_PI / 180;
  b = 190 * M_PI / 180;
  ASSERT_DOUBLE_EQ(calcDiffOfRadian(a, b), 170 * M_PI / 180)
      << "diff should be " << 170 * M_PI / 180;
