...
void ObstacleStopPlannerNode::obstaclePointcloudCallback(const sensor_msgs::PointCloud2::ConstPtr& input_msg) {
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr no_height_filtered_pointcloud_ptr(new pcl::PointCloud<pcl::PointXYZ>);
...
void ObstacleStopPlannerNode::pathCallback(const autoware_planning_msgs::Trajectory::ConstPtr& input_msg) {
  ...
  trimTrajectoryWithIndexFromSelfPose(*input_msg, self_pose, trim_trajectory, trajectory_trim_index);
...
bool ObstacleStopPlannerNode::decimateTrajectory(const autoware_planning_msgs::Trajectory& input_trajectory,
  ...
  for (int i = 0; i < (int)(input_trajectory.points.size()) - 1; ++i) {
    if (next_length <= trajectory_length_sum + epsilon) {
      ...
      line_start_point << input_trajectory.points.at(i).pose.position.x, input_trajectory.points.at(i).pose.position.y;
      line_end_point << input_trajectory.points.at(i + 1).pose.position.x,
          input_trajectory.points.at(i + 1).pose.position.y;
      getBackwordPointFromBasePoint(line_start_point, line_end_point, line_end_point,
    ...
    const double x = input_trajectory.points.at(i).pose.position.x - input_trajectory.points.at(i + 1).pose.position.x;
    const double y = input_trajectory.points.at(i).pose.position.y - input_trajectory.points.at(i + 1).pose.position.y;
...
bool ObstacleStopPlannerNode::trimTrajectoryFromSelfPose(const autoware_planning_msgs::Trajectory& input_trajectory,
                                                         const geometry_msgs::Pose self_pose,
                                                         autoware_planning_msgs::Trajectory& output_trajectory) {
  size_t index;
  return trimTrajectoryWithIndexFromSelfPose(output_trajectory, self_pose, output_trajectory, index);
}
...
bool ObstacleStopPlannerNode::convexHull(const std::vector<cv::Point2d> pointcloud,
  ...
  for (size_t i = 0; i < pointcloud.size(); ++i) {
    normalized_pointcloud.push_back(
        cv::Point((pointcloud.at(i).x - centroid.x) * 1000.0, (pointcloud.at(i).y - centroid.y) * 1000.0));
  }
...
bool ObstacleStopPlannerNode::getSelfPose(const std_msgs::Header& header, const tf2_ros::Buffer& tf_buffer,
                                          geometry_msgs::Pose& self_pose) {
  try {
    geometry_msgs::TransformStamped transform;
    transform = tf_buffer.lookupTransform(header.frame_id, "base_link", header.stamp, ros::Duration(0.1));
...
bool ObstacleStopPlannerNode::getBackwordPointFromBasePoint(const Eigen::Vector2d& line_point1,
...
void ObstacleStopPlannerNode::getNearestPoint(const pcl::PointCloud<pcl::PointXYZ>& pointcloud,
  ...
  for (size_t i = 0; i < pointcloud.size(); ++i) {
    ...
    pointcloud_vec << pointcloud.at(i).x - base_pose.position.x, pointcloud.at(i).y - base_pose.position.y;
