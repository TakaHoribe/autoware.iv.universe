...
loose_constrain_disntance_(0.4),
    ...
    std::vector<geometry_msgs::Point>& debug_constrain_points)
{
  ...
  // std::cout << "num fixed "<< num_fixed_points << std::endl;
...
void EBPathSmoother::solveQP(const Mode qp_mode)
{
  ...
  {
    osqp_update_eps_rel(&workspace, 1e-7f);
  }
  else
  {
    osqp_update_eps_rel(&workspace, 1e-7f);
  }
  osqp_update_eps_abs(&workspace, 1e-6f);
  osqp_update_alpha(&workspace, 1.6);
  osqp_update_max_iter(&workspace, 100000);
