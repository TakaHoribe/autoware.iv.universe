...
MotionVelocityOptimizer::MotionVelocityOptimizer() : nh_(""), pnh_("~"), tf_listener_(tf_buffer_) {
  ...
  sub_current_trajectory_ =
      pnh_.subscribe("input/trajectory", 1, &MotionVelocityOptimizer::callbackCurrentTrajectory, this);
  sub_current_velocity_ =
      pnh_.subscribe("/vehicle/status/twist", 1, &MotionVelocityOptimizer::callbackCurrentVelocity, this);
  sub_external_velocity_limit_ =
      pnh_.subscribe("external_velocity_limit_mps", 1, &MotionVelocityOptimizer::callbackExternalVelocityLimit, this);

  /* dynamic reconfigure */
  dynamic_reconfigure::Server<motion_velocity_optimizer::MotionVelocityOptimizerConfig>::CallbackType dyncon_f =
  ...
  pub_trajectory_vel_lim_ =
      pnh_.advertise<autoware_planning_msgs::Trajectory>("debug/trajectory_external_velocity_limitted", 1);
  ...
  while (ros::ok()) {
    try {
      ...
      ROS_INFO("[MotionVelocityOptimizer] is waitting to get map to base_link transform. %s", ex.what());
      continue;
    }
  }
}
...
void MotionVelocityOptimizer::publishTrajectory(const autoware_planning_msgs::Trajectory& trajectory) const {
  pub_trajectory_.publish(trajectory);
}

void MotionVelocityOptimizer::callbackCurrentVelocity(const geometry_msgs::TwistStamped::ConstPtr msg) {
  current_velocity_ptr_ = msg;
}
void MotionVelocityOptimizer::callbackCurrentTrajectory(const autoware_planning_msgs::Trajectory::ConstPtr msg) {
...
autoware_planning_msgs::Trajectory MotionVelocityOptimizer::calcTrajectoryVelocity(
  ...
  int input_closest = vpu::calcClosestWaypoint(traj_input, current_pose_ptr_->pose, planning_param_.delta_yaw_threshold);
  if (input_closest < 0) {
    ROS_WARN("[velocity planner] cannot find closest waypoint for input trajectory");
    return prev_output_;
  }

  autoware_planning_msgs::Trajectory traj_extracted;        // extructed from traj_input around current_position
  autoware_planning_msgs::Trajectory traj_vel_limtted;      // velocity is limitted by external velocity limit
  autoware_planning_msgs::Trajectory traj_latacc_filtered;  // velocity is limitted by max lateral acceleration
  ...
  int traj_resampled_closest = vpu::calcClosestWaypoint(traj_resampled, current_pose_ptr_->pose, planning_param_.delta_yaw_threshold);
  ...
  const bool is_reverse_velocity = (traj_resampled.points.at(traj_resampled_closest).twist.linear.x < 0.0);
  ...
  int prev_output_closest = vpu::calcClosestWaypoint(prev_output_, current_pose_ptr_->pose, planning_param_.delta_yaw_threshold);
...
bool MotionVelocityOptimizer::resampleTrajectory(const autoware_planning_msgs::Trajectory& input,
  ...
  if (!vpu::linearInterpTrajectory(in_arclength, input, out_arclength, output)) {
    ROS_WARN(
        "[motion_velocity_optimizer]: fail trajectory interpolation. size : in_arclength = %lu, input = %lu, "
...
void MotionVelocityOptimizer::calcInitialMotion(const double& target_vel,
  ...
  if (vehicle_speed < planning_param_.engage_velocity && target_vel > planning_param_.engage_velocity) {
    ...
    const double stop_dist =
        vpu::calcDist2d(reference_traj.points.at(idx), reference_traj.points.at(reference_traj_closest));
...
void MotionVelocityOptimizer::solveOptimization(const double initial_vel, const double initial_acc,
  ...
  if (std::fabs(input.points.at(closest).twist.linear.x) < 0.1) {
    DEBUG_INFO("[MotionVelocityOptimizer::solveOptimization] closest vmax < 0.1, keep stopping. return.");
    return;
  }
  ...
  for (unsigned int i = 0; i < N; ++i) {
    DEBUG_INFO_ALL("i = %d, v: %f, vmax: %f a: %f, b: %f, delta: %f, sigma: %f\n", i, std::sqrt(optval.at(i)), vmax[i],
                   optval.at(i + N), optval.at(i), optval.at(i + 2 * N), optval.at(i + 3 * N));
  }
...
void MotionVelocityOptimizer::optimizeVelocity(const autoware_planning_msgs::Trajectory& input, const int input_closest,
  ...
  calcInitialMotion(target_vel, input, input_closest, prev_output, prev_output_closest,
  ...
  DEBUG_INFO("[replan] : target_vel = %f, stop_idx_zero_vel = %d, input_closest = %d, stop_point_exists = %d",
...
bool MotionVelocityOptimizer::lateralAccelerationFilter(const autoware_planning_msgs::Trajectory& input,
  ...
  for (int i = 0; i < input_size; ++i) {
    ...
    const double v_curvature_max =
        std::max(std::sqrt(max_lateral_accel_abs / std::max(curvature, 1.0E-5)), planning_param_.min_curve_velocity);
  ...
  if (std::fabs(current_velocity_ptr_->twist.linear.x) < 0.1) {
    ...
    if (stop_point_exist && stop_idx >= closest /* desired stop line is ahead of ego-vehicle */) {
      ...
      if (dist_to_stopline < planning_param_.stop_dist_to_prohibit_engage) {
        ...
        DEBUG_INFO(
            "[preventMoveToVeryCloseStopLine] vehicle velocity is low, and stop point is very close. keep stopping."
            " curr_vel = %3.3f, dist_to_stopline = %3.3f < move_dist_min = %3.3f, stop_idx = %d, closest = %d",
            current_velocity_ptr_->twist.linear.x, dist_to_stopline, planning_param_.stop_dist_to_prohibit_engage,
            stop_idx, closest);
        return;
      }
    }
    DEBUG_INFO(
        ...
        " curr_vel = %3.3f, dist_to_stopline = %3.3f < move_dist_min = %3.3f, stop_idx = %d, closest = %d",
        current_velocity_ptr_->twist.linear.x, dist_to_stopline, planning_param_.stop_dist_to_prohibit_engage, stop_idx,
        closest);
  }
}
...
bool MotionVelocityOptimizer::extractPathAroundIndex(const autoware_planning_msgs::Trajectory& input, const int index,
  ...
  if (input.points.size() == 0 || index < 0 || (int)input.points.size() - 1 < index) {
    ROS_WARN("extractPathAroundIndex failed. input.points.size() = %lu, base_index = %d", input.points.size(), index);
    return false;
  }
  ...
  DEBUG_INFO("[extractPathAroundIndex] : input.size() = %lu, extract_base_index = %d, output.size() = %lu",
