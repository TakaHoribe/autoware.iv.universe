...
/* read current time */
void tic(PyTimer* t) { clock_gettime(CLOCK_MONOTONIC, &t->tic); }
...
static PyObject* OSQP_update_bounds(PyObject* self, PyObject* args) {
  ...
  // Parse arguments
  if (!PyArg_ParseTuple(args, argparse_string, &PyArray_Type, &l, &PyArray_Type, &u)) {
    return NULL;
  }
...
static PyObject* OSQP_update_P_A(PyObject* self, PyObject* args) {
  ...
  // Parse arguments
  if (!PyArg_ParseTuple(args, argparse_string, &Px, &Px_idx, &Px_n, &Ax, &Ax_idx, &Ax_n)) {
    return NULL;
  }
