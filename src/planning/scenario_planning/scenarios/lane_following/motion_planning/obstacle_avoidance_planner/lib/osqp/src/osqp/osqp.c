...
c_int osqp_setup(OSQPWorkspace** workp, const OSQPData* data, const OSQPSettings* settings) {
  ...
  *workp = work;
  ...
  if (data->m && (!(work->data->l) || !(work->data->u))) return osqp_error(OSQP_MEM_ALLOC_ERROR);

  // Vectorized rho parameter
  work->rho_vec = c_malloc(data->m * sizeof(c_float));
  work->rho_inv_vec = c_malloc(data->m * sizeof(c_float));
  if (data->m && (!(work->rho_vec) || !(work->rho_inv_vec))) return osqp_error(OSQP_MEM_ALLOC_ERROR);
  ...
  if (!(work->x) || !(work->xz_tilde) || !(work->x_prev)) return osqp_error(OSQP_MEM_ALLOC_ERROR);
  if (data->m && (!(work->z) || !(work->z_prev) || !(work->y))) return osqp_error(OSQP_MEM_ALLOC_ERROR);
  ...
  if (data->m && (!(work->Ax) || !(work->delta_y) || !(work->Adelta_x))) return osqp_error(OSQP_MEM_ALLOC_ERROR);
  ...
  if (settings->scaling) {
    ...
    if (!(work->scaling->D) || !(work->scaling->Dinv)) return osqp_error(OSQP_MEM_ALLOC_ERROR);
    if (data->m && (!(work->scaling->E) || !(work->scaling->Einv))) return osqp_error(OSQP_MEM_ALLOC_ERROR);
  ...
  exitflag = init_linsys_solver(&(work->linsys_solver), work->data->P, work->data->A, work->settings->sigma,
  ...
  if (data->m && (!(work->pol->Alow_to_A) || !(work->pol->Aupp_to_A) || !(work->pol->A_to_Alow) ||
                  !(work->pol->A_to_Aupp) || !(work->pol->z) || !(work->pol->y)))
    return osqp_error(OSQP_MEM_ALLOC_ERROR);
...
c_int osqp_solve(OSQPWorkspace* work) {
...
#ifdef PROFILING
  if (work->clear_update_time == 1) work->info->update_time = 0.0;
    ...
    if (work->settings->adaptive_rho && !work->settings->adaptive_rho_interval) {
      ...
      if (osqp_toc(work->timer) > work->settings->adaptive_rho_fraction * work->info->setup_time) {
        ...
        work->settings->adaptive_rho_interval =
            c_max(work->settings->adaptive_rho_interval, work->settings->check_termination);
  ...
  if (work->settings->polish && (work->info->status_val == OSQP_SOLVED)) polish(work);
#endif /* ifndef EMBEDDED */
...
c_int osqp_update_P(OSQPWorkspace* work, const c_float* Px_new, const c_int* Px_new_idx, c_int P_new_n) {
  ...
  if (Px_new_idx) {  // Passing the index of elements changed
    ...
    if (P_new_n > nnzP) {
#ifdef PRINTING
      c_eprint("new number of elements (%i) greater than elements in P (%i)", (int)P_new_n, (int)nnzP);
#endif /* ifdef PRINTING */
      return 1;
    }
  }
...
c_int osqp_update_A(OSQPWorkspace* work, const c_float* Ax_new, const c_int* Ax_new_idx, c_int A_new_n) {
  ...
  if (Ax_new_idx) {  // Passing the index of elements changed
    ...
    if (A_new_n > nnzA) {
#ifdef PRINTING
      c_eprint("new number of elements (%i) greater than elements in A (%i)", (int)A_new_n, (int)nnzA);
#endif /* ifdef PRINTING */
      return 1;
    }
  }
...
c_int osqp_update_P_A(OSQPWorkspace* work, const c_float* Px_new, const c_int* Px_new_idx, c_int P_new_n,
                      const c_float* Ax_new, const c_int* Ax_new_idx, c_int A_new_n) {
  c_int i;           // For indexing
  c_int exitflag;    // Exit flag
...
#ifdef PROFILING
  if (work->rho_update_from_solve == 0) work->info->update_time += osqp_toc(work->timer);
#endif /* ifdef PROFILING */
