...
void update_x(OSQPWorkspace* work) {
  ...
  // update x
  for (i = 0; i < work->data->n; i++) {
    work->x[i] = work->settings->alpha * work->xz_tilde[i] + ((c_float)1.0 - work->settings->alpha) * work->x_prev[i];
  }
...
void update_z(OSQPWorkspace* work) {
  ...
  for (i = 0; i < work->data->m; i++) {
    work->z[i] = work->settings->alpha * work->xz_tilde[i + work->data->n] +
                 ((c_float)1.0 - work->settings->alpha) * work->z_prev[i] + work->rho_inv_vec[i] * work->y[i];
  }
...
void update_y(OSQPWorkspace* work) {
  ...
  for (i = 0; i < work->data->m; i++) {
    work->delta_y[i] = work->rho_vec[i] * (work->settings->alpha * work->xz_tilde[i + work->data->n] +
...
c_float compute_obj_val(OSQPWorkspace* work, c_float* x) {
  ...
  obj_val = quad_form(work->data->P, x) + vec_prod(work->data->q, x, work->data->n);
...
c_int is_dual_infeasible(OSQPWorkspace* work, c_float eps_dual_inf) {
  ...
  if (norm_delta_x > eps_dual_inf) {
    ...
    if (vec_prod(work->data->q, work->delta_x, work->data->n) < -cost_scaling * eps_dual_inf * norm_delta_x) {
      ...
      if (vec_norm_inf(work->Pdelta_x, work->data->n) < cost_scaling * eps_dual_inf * norm_delta_x) {
        // Compute A * delta_x
...
c_int has_solution(OSQPInfo* info) {
  return ((info->status_val != OSQP_PRIMAL_INFEASIBLE) && (info->status_val != OSQP_PRIMAL_INFEASIBLE_INACCURATE) &&
          (info->status_val != OSQP_DUAL_INFEASIBLE) && (info->status_val != OSQP_DUAL_INFEASIBLE_INACCURATE) &&
...
void store_solution(OSQPWorkspace* work) {
  ...
  if (has_solution(work->info)) {
    ...
    if (work->settings->scaling) unscale_solution(work);
  } else {
...
void update_status(OSQPInfo* info, c_int status_val) {
  ...
  if (status_val == OSQP_SOLVED_INACCURATE)
    ...
    c_strcpy(info->status, "problem non convex");
}
...
#ifdef PRINTING
    c_eprint("n must be positive and m nonnegative; n = %i, m = %i", (int)data->n, (int)data->m);
#endif /* ifdef PRINTING */
    return 1;
  }
...
#ifdef PRINTING
      c_eprint("Lower bound at index %d is greater than upper bound: %.4e > %.4e", (int)j, data->l[j], data->u[j]);
#endif /* ifdef PRINTING */
      return 1;
    }
  }
...
c_int validate_linsys_solver(c_int linsys_solver) {
  if ((linsys_solver != QDLDL_SOLVER) && (linsys_solver != MKL_PARDISO_SOLVER)) {
    return 1;
  }
