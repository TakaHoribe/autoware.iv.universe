...
static c_int LDL_factor(csc* A, qdldl_solver* p, c_int nvar) {
...
#ifdef PRINTING
    c_eprint(
        "Error in KKT matrix LDL factorization when computing the elimination tree. A is not perfectly upper "
        "triangular");
#endif
    return sum_Lnz;
  }
  ...
  // Factor matrix
  factor_status = QDLDL_factor(A->n, A->p, A->i, A->x, p->L->p, p->L->i, p->L->x, p->D, p->Dinv, p->Lnz, p->etree,
                               p->bwork, p->iwork, p->fwork);

  if (factor_status < 0) {
    // Error
#ifdef PRINTING
    c_eprint(
        "Error in KKT matrix LDL factorization when computing the nonzero elements. There are zeros in the diagonal "
        "matrix");
#endif
...
#ifdef PRINTING
    c_eprint(
        "Error in KKT matrix LDL factorization when computing the nonzero elements. The problem seems to be "
        "non-convex");
#endif
    return -2;
  }
...
static c_int permute_KKT(csc** KKT, qdldl_solver* p, c_int Pnz, c_int Anz, c_int m, c_int* PtoKKT, c_int* AtoKKT,
                         c_int* rhotoKKT) {
  c_float* info;
  c_int amd_status;
  c_int* Pinv;
...
c_int init_linsys_solver_qdldl(qdldl_solver** sp, const csc* P, const csc* A, c_float sigma, const c_float* rho_vec,
                               c_int polish) {
  // Define Variables
  ...
  *sp = s;
  ...
  if (polish) {  // Called from polish()
    ...
    // Permute matrix
    if (KKT_temp) permute_KKT(&KKT_temp, s, OSQP_NULL, OSQP_NULL, OSQP_NULL, OSQP_NULL, OSQP_NULL, OSQP_NULL);
    ...
    KKT_temp =
        form_KKT(P, A, 0, sigma, s->rho_inv_vec, s->PtoKKT, s->AtoKKT, &(s->Pdiag_idx), &(s->Pdiag_n), s->rhotoKKT);

    // Permute matrix
    if (KKT_temp) permute_KKT(&KKT_temp, s, P->p[P->n], A->p[A->n], A->m, s->PtoKKT, s->AtoKKT, s->rhotoKKT);
  }
...
c_int update_linsys_solver_matrices_qdldl(qdldl_solver* s, const csc* P, const csc* A) {
  ...
  return (QDLDL_factor(s->KKT->n, s->KKT->p, s->KKT->i, s->KKT->x, s->L->p, s->L->i, s->L->x, s->D, s->Dinv, s->Lnz,
