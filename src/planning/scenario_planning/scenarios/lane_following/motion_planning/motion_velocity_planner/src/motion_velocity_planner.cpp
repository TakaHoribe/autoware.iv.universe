...
MotionVelocityPlanner::MotionVelocityPlanner() : nh_(""), pnh_("~"), tf_listener_(tf_buffer_) {
  ...
  sub_current_trajectory_ =
      pnh_.subscribe("input/trajectory", 1, &MotionVelocityPlanner::callbackCurrentTrajectory, this);
  sub_current_velocity_ =
      pnh_.subscribe("/vehicle/status/twist", 1, &MotionVelocityPlanner::callbackCurrentVelocity, this);
  sub_external_velocity_limit_ =
      pnh_.subscribe("/external_velocity_limit_mps", 1, &MotionVelocityPlanner::callbackExternalVelocityLimit, this);
...
void MotionVelocityPlanner::timerReplanCallback(const ros::TimerEvent& e) {
  ...
  if (!vpu::extractPathAroundIndex(*base_traj_raw_ptr_, base_raw_closest, planning_param_.extract_ahead_dist,
    ...
    ROS_WARN(
        "[motion_velocity_planner] extractPathAroundIndex failed. base_traj_raw_ptr_->size() = %lu, "
  ...
  DEBUG_INFO(
      "[extractPathAroundIndex] : base_raw.size() = %lu, base_closest = %d, "
  ...
  if (external_velocity_limit_ptr_ != nullptr) {
    DEBUG_INFO("[motion_velocity_planner] : apply external velocity lim : %3.3f, base_closest_vel = %3.3f",
               external_velocity_limit_ptr_->data, base_traj_raw_ptr_->points.at(base_raw_closest).twist.linear.x);
...
void MotionVelocityPlanner::publishStopDistance(const autoware_planning_msgs::Trajectory& trajectory,
...
void MotionVelocityPlanner::calcInitialMotion(const double& base_speed,
                                              const autoware_planning_msgs::Trajectory& base_waypoints,
                                              const int base_closest,
                                              const std::vector<Motion>& prev_replanned_traj_motion,
                                              const int prev_replanned_traj_closest,
  ...
  if (std::fabs(vel_error) > planning_param_.replan_vel_deviation /* when velocity tracking deviation is large */) {
    ...
    init_type = 1;
    DEBUG_WARN(
        "[calcInitialMotion] : Large deviation error for speed control. Use current speed for initial value, "
  ...
  if (vehicle_speed < 0.5 * planning_param_.engage_velocity &&
      base_speed - vehicle_speed > planning_param_.engage_velocity) {
    int idx = 0;
    ...
    const double stop_dist =
        std::sqrt(vpu::calcSquaredDist2d(base_waypoints.points.at(idx), base_waypoints.points.at(base_closest)));
    if (!exist_stop_point || stop_dist > planning_param_.stop_dist_to_prohibit_engage) {
      ...
      DEBUG_INFO("[calcInitialMotion] : engage condition, but stop point is close (dist = %3.3f). no engage.",
                 stop_dist);
    }
  }
  ...
  init_type = 3;
  DEBUG_INFO(
      "[calcInitialMotion]: normal update initial_motion.vel = %f, acc = %f, jerk = %f, vehicle_speed = %f, "
      "base_speed = %f",
...
void MotionVelocityPlanner::replanVelocity(const autoware_planning_msgs::Trajectory& input, const int input_closest,
                                           ...
                                           std::vector<Motion>& output_motion, double& stop_planning_jerk) const {
  ...
  {
    if (!stopVelocityFilterWithJerkRange(planning_param_.dec_jerk_urgent, planning_param_.dec_jerk_nominal,
                                         planning_param_.jerk_planning_span, planning_param_.stop_dist_mergin,
                                         stop_idx_zero_vel, jerk_filtered_traj, jerk_filtered_motion, input_closest,
                                         /* out */ stop_planning_jerk, /* out */ output, /* out */ is_stop_ok,
                                         /* out */ output_motion)) {
    ...
    {
      setZeroLaneAndMotions(input, output, output_motion);
      if (std::fabs(current_velocity_ptr_->twist.linear.x) >
          planning_param_.emergency_flag_vel_thr_kmph / 3.6 /* 3.0 km/h */) {
        ...
        ROS_WARN_DELAYED_THROTTLE(5.0,
                                  "[replan] : could not plan under given jerk constraint. stop_idx = %d, self_idx = "
                                  "%d, dist = %f, v0 = %f, a0 "
                                  "= %f",
  ...
  } else /* stop idx is behind self pose -> emergency */
  {
    ...
    DEBUG_INFO("[replan] : over stop line. publish zero velocity. stop_idx_zero_vel = %d, input_closest = %d",
               stop_idx_zero_vel, input_closest);
  }
  ...
  if (output.points.size() > 0) output.points.back().twist.linear.x = 0.0;
  if (output_motion.size() > 0) output_motion.back().vel = 0.0;
...
void MotionVelocityPlanner::setZeroLaneAndMotions(const autoware_planning_msgs::Trajectory& base_trajectory,
...
bool MotionVelocityPlanner::moveAverageFilter(const autoware_planning_msgs::Trajectory& input,
                                              const unsigned int move_ave_num,
...
MotionVelocityPlanner::Motion MotionVelocityPlanner::updateMotionWithConstraint(
    const Motion& m_prev, const Motion& m_des_prev, const double& dt, const MotionVelocityPlannerParam& planning_param,
    int& debug) const {
  debug = 0;
  ...
  if (planning_param.min_decel < m_des_prev.acc && m_des_prev.acc < planning_param.max_accel &&
      curr_acc_min < m_des_prev.acc && m_des_prev.acc < curr_acc_max) {
...
void MotionVelocityPlanner::jerkVelocityFilter(const Motion& initial_motion,
                                               const autoware_planning_msgs::Trajectory& input, const int input_closest,
                                               const std::vector<Motion>& prev_output_motion,
                                               const int prev_output_closest, const int& init_type,
                                               autoware_planning_msgs::Trajectory& output,
  ...
  DEBUG_INFO("[jerkVelocityFilter]: constraint acc = %f, decel = %f, acc_jerk = %f, dec_jerk = %f",
             planning_param_.max_accel, planning_param_.min_decel, planning_param_.acc_jerk,
             planning_param_.dec_jerk_nominal);
  ...
  for (int i = idx + 1; i < (int)input.points.size(); ++i) {
    ...
    DEBUG_INFO_ALL(
        "i = %d, ret.v = %f, ret.a = %f, ret.s = %f, prev.v = %f, prev.a = %f, prev.s = %f, des.v = %f, "
        "des.acc = %f, des.s = %f, dt_prev = %f, kp = %f, debug = %d",
        i, m_curr.vel, m_curr.acc, m_curr.jerk, m_prev.vel, m_prev.acc, m_prev.jerk, m_des_prev.vel, m_des_prev.acc,
        m_des_prev.jerk, dt_prev, kp, debug);
...
bool MotionVelocityPlanner::stopVelocityFilterWithJerkRange(
    const double& jerk_max, const double& jerk_min, const double& jerk_span, const double& stop_mergin,
    const int& input_stop_idx, const autoware_planning_msgs::Trajectory& input, const std::vector<Motion>& input_motion,
    const int& input_closest, double& planning_jerk, autoware_planning_msgs::Trajectory& output, bool& is_stop_ok,
    std::vector<Motion>& output_motion) const {
  ...
  for (double stop_planning_jerk = jerk_min; stop_planning_jerk > jerk_max - ep; stop_planning_jerk -= jerk_span) {
    if (!stopVelocityFilterWithMergin(stop_mergin, input_stop_idx, input, input_motion, input_closest,
                                      stop_planning_jerk, /* out */ output,
...
bool MotionVelocityPlanner::stopVelocityFilterWithMergin(const double& stop_mergin, const int& input_stop_idx,
                                                         ...
                                                         const std::vector<Motion>& input_motion,
                                                         const int& input_closest, const double& planning_jerk,
                                                         autoware_planning_msgs::Trajectory& output, bool& is_stop_ok,
                                                         std::vector<Motion>& output_motion) const {
  ...
  for (int i = input_stop_idx_with_mergin; i <= input_stop_idx; ++i) {
    ...
    if (is_stop_ok) {
      ...
      DEBUG_INFO("[stopVelocityFilterWithMergin] plan successed, planning_jerk = %f, mergin = %f", planning_jerk,
                 mergin);
...
bool MotionVelocityPlanner::stopVelocityFilter(const int& input_stop_idx,
                                               const autoware_planning_msgs::Trajectory& input,
  ...
  if (!vpu::calcStopDistWithConstantJerk(v0_s, a0_s, planning_jerk, v_end_s, t_neg_s, t_pos_s, brake_dist_s)) {
    ROS_WARN_DELAYED_THROTTLE(
        3.0, "[stopVelocityFilter]: cannot calculate stop_dist in calcStopDistWithConstantJerk() (planning_jerk = %f)",
        planning_jerk);
    is_stop_ok = false;
    return true;
  }
  DEBUG_INFO(
      "[stopVelocityFilter]: check if distance is enough : jerk = %3.3f, v0 = %3.3f, a0 = %3.3f, needed_dist = %3.3f, "
      "dist_to_stopline = %3.3f, stop_idx = %d, closest = %d",
  ...
  if (!is_stop_ok) return true;  // this should be true (is_stop_ok = false)
  ...
  for (int i = input_stop_idx - 1; i >= input_closest; --i) {
    ...
    if (vpu::calcStopDistWithConstantJerk(v0, a0, planning_jerk, v_end_s, t_neg, t_pos, brake_dist)) {
      // printf("[stopVelocityFilter]: searching start idx for stop planning : i = %d, v = %3.3f, a = %3.3f, brake_dist
      // = %3.3f, \n", i, v0, a0, brake_dist);
      ...
      if (start_point_found) {
        ...
        DEBUG_INFO(
            "[stopVelocityFilter]: stop planning start point found : jerk = %3.3f, start_idx = %d"
  ...
  if (plan_start_index == -1) {
    ROS_ERROR("stop_dist is enough, but could not find stop point. Something wrong. (planning_jerk = %f)",
              planning_jerk);
    return false;
  }
...
void MotionVelocityPlanner::calculateMotionsFromWaypoints(const autoware_planning_msgs::Trajectory& trajectory,
                                                          std::vector<Motion> motions) const {
  std::vector<double> s;
...
void MotionVelocityPlanner::preventMoveToVeryCloseStopLine(const int closest,
  ...
  if (std::fabs(current_velocity_ptr_->twist.linear.x) < 0.01) {
    ...
    if (stop_point_exist && stop_idx >= closest /* desired stop line is ahead of ego-vehicle */) {
      double stop_dist =
          std::sqrt(vpu::calcSquaredDist2d(trajectory.points.at(stop_idx), trajectory.points.at(closest)));
