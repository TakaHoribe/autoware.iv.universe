...
class MotionVelocityPlanner {
 ...
 private:
  ...
  struct MotionVelocityPlannerParam {
    ...
    double emergency_flag_vel_thr_kmph;   // Threshold for throwing emergency flag when unable to stop under jerk
                                          // constraints
  ...
  void plotWaypoint(const autoware_planning_msgs::Trajectory& trajectory, const std::string& color_str,
                    const std::string& label_str) const;
  bool moveAverageFilter(const autoware_planning_msgs::Trajectory& latacc_filtered_traj,
  ...
  void jerkVelocityFilter(const Motion& initial_motion, const autoware_planning_msgs::Trajectory& input,
                          const int input_closest, const std::vector<Motion>& prev_output_motion,
                          const int prev_output_closest, const int& init_type,
                          autoware_planning_msgs::Trajectory& output, std::vector<Motion>& output_motion) const;
  ...
  void calculateMotionsFromWaypoints(const autoware_planning_msgs::Trajectory& trajectory,
  ...
  void setZeroLaneAndMotions(const autoware_planning_msgs::Trajectory& base_trajectory,
                             autoware_planning_msgs::Trajectory& trajectory, std::vector<Motion>& motions) const;
  ...
  class EmergencyStopManager {
   ...
   public:
    EmergencyStopManager(const MotionVelocityPlanner* motion_velocity_planner_ptr, const double& safety_time_for_stop)
      ...
      timer_ = motion_velocity_planner_ptr_->nh_.createTimer(
          ros::Duration(0.1), &MotionVelocityPlanner::EmergencyStopManager::timerCallback, this);
      is_emergency_ = false;
      is_counting_ = false;
    };
