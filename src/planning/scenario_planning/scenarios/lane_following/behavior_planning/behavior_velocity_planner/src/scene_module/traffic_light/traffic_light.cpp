...
bool TrafficLightModule::run(const autoware_planning_msgs::PathWithLaneId &input, autoware_planning_msgs::PathWithLaneId &output)
{
    ...
    else if (state_ == State::APPROARCH)
    {
        ...
        // update state
        {
            ...
            if (!createTargetPoint(input, stop_line, -2.0 /*overline margin*/, judge_point_idx, judge_point))
                return false;
            const double sq_dist = (judge_point.x() - self_pose.pose.position.x) * (judge_point.x() - self_pose.pose.position.x) +
            ...
            if (sq_dist < range * range)
            {
                ...
                tf2::Quaternion quat;
                quat.setRPY(0, 0, yaw);
                tf2::Transform tf_map2judge_pose(quat, tf2::Vector3(judge_point.x(), judge_point.y(), 0.0));
        ...
        if (!createTargetPoint(input, stop_line, stop_margin_, stop_line_point_idx, stop_line_point))
            return false;
...
bool TrafficLightModule::getHighestConfidenceTrafficLightState(lanelet::ConstLineStringsOrPolygons3d &traffic_lights,
                                                               autoware_traffic_light_msgs::TrafficLightState &highest_confidence_tl_state)
{
    // search traffic light state
    bool found = false;
...
bool TrafficLightModule::createTargetPoint(const autoware_planning_msgs::PathWithLaneId &input,
                                          const boost::geometry::model::linestring<boost::geometry::model::d2::point_xy<double>> &stop_line,
                                          const double &margin,
                                          ...
                                          Eigen::Vector2d &target_point)
{
    for (size_t i = 0; i < input.points.size() - 1; ++i)
    {
        ...
        {
            double dist = bg::distance(Point(input.points.at(i).point.pose.position.x, input.points.at(i).point.pose.position.y),
        ...
        getBackwordPointFromBasePoint(point2,
                                      point1,
                                      point2,
                                      std::fabs(length_sum - target_length),
                                      target_point);
        return true;
    }
    return false;
