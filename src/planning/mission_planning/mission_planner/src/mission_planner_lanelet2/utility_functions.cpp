...
bool exists(const std::unordered_set<lanelet::Id>& set, const lanelet::Id& id) { return set.find(id) != set.end(); }
...
bool getClosestLanelet(const geometry_msgs::Pose& search_pose, const lanelet::LaneletMapPtr& lanelet_map_ptr_,
  ...
  if (nearest_lanelet.empty()) {
    ROS_ERROR_STREAM("Failed to find the closest lane!" << std::endl
                                                        << "search point: " << toString(search_pose) << std::endl);
    return false;
  }
  if (nearest_lanelet.front().first > distance_thresh) {
    ROS_ERROR_STREAM("Closest lane is too far away!" << std::endl
                                                     << "search point: " << toString(search_pose) << std::endl
