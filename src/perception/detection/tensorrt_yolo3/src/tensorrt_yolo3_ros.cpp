...
//   std::cerr << "*******"  << std::endl;
  ...
  {
      cv::rectangle(in_image_ptr->image,cv::Point(item.left,item.top),cv::Point(item.right,item.bot),cv::Scalar(0,0,255),3,8,0);
  }
                                                      ...
                                                      autoware_perception_msgs::DynamicObjectWithFeatureArray& out_objects)
{
    ...
    if(nms_thresh > 0) 
        doNms(detections,classes,nms_thresh);
    ...
    for(const auto& item : detections)
    {
        ...
        double roi_height = std::min(int((b[1]+b[3]/2.)*height),height) - std::max(int((b[1]-b[3]/2.)*height),0);
