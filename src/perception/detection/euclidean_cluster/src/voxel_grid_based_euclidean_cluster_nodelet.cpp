...
void VoxelGridBasedEuclideanClusterNodelet::pointcloudCallback(const sensor_msgs::PointCloud2ConstPtr &input_msg)
{
    ...
    {
        const int index =
            voxel_grid_.getCentroidIndexAt(voxel_grid_.getGridCoordinates(raw_pointcloud_ptr->points.at(i).x, raw_pointcloud_ptr->points.at(i).y, raw_pointcloud_ptr->points.at(i).z));
    ...
    // build debug msg
    if (debug_pub_.getNumSubscribers() < 1)
        return;
    {
