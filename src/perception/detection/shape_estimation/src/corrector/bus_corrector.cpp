...
bool BusCorrector::correct(autoware_perception_msgs::Shape &shape_output,
                           geometry_msgs::Pose &pose_output,
                           bool &orientaion_output)
{
    Eigen::Translation<double, 2> trans = Eigen::Translation<double, 2>(pose_output.position.x, pose_output.position.y);

    Eigen::Quaterniond quat(pose_output.orientation.w, pose_output.orientation.x, pose_output.orientation.y, pose_output.orientation.z);
    ...
    */
    ...
    size_t third_most_distant_index;
    {
        ...
        for (size_t i = 0; i < v_point.size(); ++i)
        {
            if ((distance < (affine_mat * v_point.at(i)).norm()) && i != first_most_distant_index && i != second_most_distant_index)
            {
                distance = (affine_mat * v_point.at(i)).norm();
    ...
    {
        if (min_width < (v_point.at(first_most_distant_index) * 2.0).norm() && (v_point.at(first_most_distant_index) * 2.0).norm() < max_width)
        {
            if ((v_point.at(third_most_distant_index) * 2.0).norm() < max_length)
            {
                ...
                if (correction_vector.x() == 0.0)
                    correction_vector.y() =
                        std::max(std::abs(correction_vector.y()), ((min_length + max_length) / 2.0) / 2.0) * (correction_vector.y() < 0.0 ? -1.0 : 1.0) - correction_vector.y();
                else if (correction_vector.y() == 0.0)
                    correction_vector.x() =
                        std::max(std::abs(correction_vector.x()), ((min_length + max_length) / 2.0) / 2.0) * (correction_vector.x() < 0.0 ? -1.0 : 1.0) - correction_vector.x();
                else
                    return false;
            }
            else
        ...
        else if (min_length < (v_point.at(first_most_distant_index) * 2.0).norm() && (v_point.at(first_most_distant_index) * 2.0).norm() < max_length)
        {
            if ((v_point.at(third_most_distant_index) * 2.0).norm() < max_width)
            {
                ...
                if (correction_vector.x() == 0.0)
                    correction_vector.y() =
                        std::max(std::abs(correction_vector.y()), ((min_width + max_width) / 2.0) / 2.0) * (correction_vector.y() < 0.0 ? -1.0 : 1.0) - correction_vector.y();
                else if (correction_vector.y() == 0.0)
                    correction_vector.x() =
                        std::max(std::abs(correction_vector.x()), ((min_width + max_width) / 2.0) / 2.0) * (correction_vector.x() < 0.0 ? -1.0 : 1.0) - correction_vector.x();
                else
                    return false;
            }
            else
    ...
    // fit width
    else if ((min_width < (v_point.at(first_most_distant_index) * 2.0).norm() && (v_point.at(first_most_distant_index) * 2.0).norm() < max_width) &&
             (min_width < (v_point.at(second_most_distant_index) * 2.0).norm() && (v_point.at(second_most_distant_index) * 2.0).norm() < max_width))
    {
    ...
    else if (min_width < (v_point.at(second_most_distant_index) * 2.0).norm() && (v_point.at(second_most_distant_index) * 2.0).norm() < max_width)
    {
        correction_vector = v_point.at(first_most_distant_index);
    ...
    else if ((min_length < (v_point.at(second_most_distant_index) * 2.0).norm() && (v_point.at(second_most_distant_index) * 2.0).norm() < max_length) &&
