...
bool VehicleMapCorrector::correct(const VectorMap &vector_map,
                                  ...
                                  bool &orientaion_output)
{
    ...
    if (vector_map.getyaw(point.x, point.y, map_coord_vector_yaw))
    {
        ...
        tf2::Matrix3x3(input_quaternion)
        ...
        if (shape_output.dimensions.x < shape_output.dimensions.y)
        {
            ...
            double diff_90(std::atan2(std::sin(vector_map_yaw - input_yaw_90_rotated), std::cos(vector_map_yaw - input_yaw_90_rotated)));
            double diff_270(std::atan2(std::sin(vector_map_yaw - input_yaw_270_rotated), std::cos(vector_map_yaw - input_yaw_270_rotated)));
            if (rad_threshold_ < std::min(std::fabs(diff_90), std::fabs(diff_270)))
                return false;
        ...
        else
        {
            ...
            double diff_0(std::atan2(std::sin(vector_map_yaw - input_yaw_0_rotated), std::cos(vector_map_yaw - input_yaw_0_rotated)));
            double diff_180(std::atan2(std::sin(vector_map_yaw - input_yaw_180_rotated), std::cos(vector_map_yaw - input_yaw_180_rotated)));
            if (rad_threshold_ < std::min(std::fabs(diff_0), std::fabs(diff_180)))
                return false;
