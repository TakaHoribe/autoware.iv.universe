...
FeatureGenerator::FeatureGenerator(const int width, const int height, const int range, const bool use_intensity_feature,
  ...
  if (use_constant_feature && use_intensity_feature) {
    map_ptr_ = std::make_shared<FeatureMapWithConstantAndIntensity>(width_, height_, range_);
  } else if (use_constant_feature) {
    map_ptr_ = std::make_shared<FeatureMapWithConstant>(width_, height_, range_);
  } else if (use_intensity_feature) {
    map_ptr_ = std::make_shared<FeatureMapWithIntensity>(width_, height_, range_);
  } else {
    map_ptr_ = std::make_shared<FeatureMap>(width_, height_, range_);
  }
...
std::shared_ptr<FeatureMapInterface> FeatureGenerator::generate(
  ...
  const float inv_res_x =
      0.5 * map_ptr_->width / map_ptr_->range;
  const float inv_res_y =
      0.5 * map_ptr_->height / map_ptr_->range;
  ...
  for (size_t i = 0; i < pc_ptr->points.size(); ++i) {
    ...
    if (map_ptr_->max_height_data[idx] < pc_ptr->points[i].z) {
      ...
      if (use_intensity_feature_) map_ptr_->top_intensity_data[idx] = normalizeIntensity(pc_ptr->points[i].intensity);
    }
    map_ptr_->mean_height_data[idx] += static_cast<float>(pc_ptr->points[i].z);
    if (use_intensity_feature_) map_ptr_->mean_intensity_data[idx] += normalizeIntensity(pc_ptr->points[i].intensity);
  ...
  for (int i = 0; i < size; ++i) {
    if (map_ptr_->count_data[i] < epsilon) {
      ...
      if (use_intensity_feature_) map_ptr_->mean_intensity_data[i] /= map_ptr_->count_data[i];
