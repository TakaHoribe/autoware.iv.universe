...
void Cluster2D::cluster(const caffe::Blob<float>& category_pt_blob, const caffe::Blob<float>& instance_pt_blob,
  ...
  const float* instance_pt_y_data = instance_pt_blob.cpu_data() + instance_pt_blob.offset(0, 1);
...
void Cluster2D::filter(const caffe::Blob<float>& confidence_pt_blob, const caffe::Blob<float>& height_pt_blob) {
...
autoware_perception_msgs::DynamicObjectWithFeature Cluster2D::obstacleToObject(const Obstacle& in_obstacle,
...
void Cluster2D::getObjects(const float confidence_thresh, const float height_thresh, const int min_pts_num,
