...
bool CNNSegmentation::segment(const pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_ptr, const pcl::PointIndices& valid_idx,
...
//    caffe::Caffe::DeviceQuery();
  ...
  cluster2d_->cluster(*category_pt_blob_, *instance_pt_blob_, pc_ptr, valid_idx, objectness_thresh,
  ...
  int min_pts_num = 3;
  cluster2d_->getObjects(confidence_thresh, height_thresh, min_pts_num, objects, message_header_);
  return true;
}
...
void CNNSegmentation::run() {
  ...
  points_sub_ = private_node_handle.subscribe(topic_src_, 1, &CNNSegmentation::pointsCallback, this);
  points_pub_ = private_node_handle.advertise<sensor_msgs::PointCloud2>("debug/points_cluster", 1);
  d_objects_pub_ = private_node_handle.advertise<autoware_perception_msgs::DynamicObjectWithFeatureArray>(
