  ...
  try {
    ...
    for (size_t i = 0; i < input_tl_roi_msg->rois.size(); ++i) {
      cv::rectangle(cv_ptr->image,
                    cv::Point(input_tl_roi_msg->rois.at(i).roi.x_offset, input_tl_roi_msg->rois.at(i).roi.y_offset),
                    cv::Point(input_tl_roi_msg->rois.at(i).roi.x_offset + input_tl_roi_msg->rois.at(i).roi.width,
                              input_tl_roi_msg->rois.at(i).roi.y_offset + input_tl_roi_msg->rois.at(i).roi.height),
                    cv::Scalar(0, 0, 255), 1, CV_AA, 0);
      cv::putText(cv_ptr->image, std::to_string(input_tl_roi_msg->rois.at(i).id),
                  cv::Point(input_tl_roi_msg->rois.at(i).roi.x_offset, input_tl_roi_msg->rois.at(i).roi.y_offset),
