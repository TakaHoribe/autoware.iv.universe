...
bool ColorClassifier::getLampState(const cv::Mat& input_image,
  ...
  if (0 < image_pub_.getNumSubscribers()) {
    ...
    cv::line(debug_image, cv::Point(0, 0), cv::Point(debug_image.cols, 0), cv::Scalar(255, 255, 255), 1, CV_AA, 0);
    cv::line(debug_image, cv::Point(0, height), cv::Point(debug_image.cols, height), cv::Scalar(255, 255, 255), 1,
             CV_AA, 0);
    cv::line(debug_image, cv::Point(0, height * 2), cv::Point(debug_image.cols, height * 2), cv::Scalar(255, 255, 255),
             1, CV_AA, 0);
    cv::line(debug_image, cv::Point(0, height * 3), cv::Point(debug_image.cols, height * 3), cv::Scalar(255, 255, 255),
             1, CV_AA, 0);

    cv::line(debug_image, cv::Point(0, 0), cv::Point(0, debug_image.rows), cv::Scalar(255, 255, 255), 1, CV_AA, 0);
    cv::line(debug_image, cv::Point(width, 0), cv::Point(width, debug_image.rows), cv::Scalar(255, 255, 255), 1, CV_AA,
             0);
    cv::line(debug_image, cv::Point(width * 2, 0), cv::Point(width * 2, debug_image.rows), cv::Scalar(255, 255, 255), 1,
             CV_AA, 0);
    cv::line(debug_image, cv::Point(width * 3, 0), cv::Point(width * 3, debug_image.rows), cv::Scalar(255, 255, 255), 1,
             CV_AA, 0);

    cv::putText(debug_image, "green", cv::Point(0, height * 1.5), cv::FONT_HERSHEY_SIMPLEX, 1.0,
                cv::Scalar(255, 255, 255), 1, CV_AA);
    cv::putText(debug_image, "yellow", cv::Point(0, height * 2.5), cv::FONT_HERSHEY_SIMPLEX, 1.0,
                cv::Scalar(255, 255, 255), 1, CV_AA);
    cv::putText(debug_image, "red", cv::Point(0, height * 3.5), cv::FONT_HERSHEY_SIMPLEX, 1.0,
                cv::Scalar(255, 255, 255), 1, CV_AA);
  ...
  const double green_ratio =
      (double)green_pixel_num / (double)(green_filtered_bin_image.rows * green_filtered_bin_image.cols);
...
bool ColorClassifier::filterHSV(const cv::Mat& input_image, cv::Mat& green_image, cv::Mat& yellow_image,
                                cv::Mat& red_image) {
  cv::Mat hsv_image;
