...
bool MapCorrectorNode::correct(autoware_perception_msgs::DynamicObjectWithFeatureArray& input) {
  ...
  try {
    tf_stamped = tf_buffer_.lookupTransform("map", input.header.frame_id, input.header.stamp, ros::Duration(1.0));
  ...
  for (const auto& input_feature_object : input.feature_objects) {
    ...
    if (corrector_ptr->correct(vector_map_, tf_stamped, feature_object.object.shape,
