...
void MapBasedDetector::cameraInfoCallback(const sensor_msgs::CameraInfo::ConstPtr& input_msg) {
  ...
  try {
    ...
    transform =
        tf_buffer_.lookupTransform("map", input_msg->header.frame_id, input_msg->header.stamp, ros::Duration(0.2));
...
bool MapBasedDetector::getTrafficLightRoi(const geometry_msgs::Pose& camera_pose,
  ...
  tf2::Transform tf_map2camera(tf2::Quaternion(camera_pose.orientation.x, camera_pose.orientation.y,
  ...
  {
    tf2::Transform tf_map2tl(tf2::Quaternion(0, 0, 0, 1), tf2::Vector3(tl_left_down_point.x(), tl_left_down_point.y(),
                                                                       tl_left_down_point.z() + tl_height));
...
void MapBasedDetector::mapCallback(const autoware_lanelet2_msgs::MapBin& input_msg) {
  ...
  for (auto tl_itr = all_lanelet_traffic_lights.begin(); tl_itr != all_lanelet_traffic_lights.end(); ++tl_itr) {
...
void MapBasedDetector::isInVisibility(const std::vector<lanelet::ConstLineString3d>& all_traffic_lights,
  ...
  for (const auto& traffic_light : all_traffic_lights) {
    ...
    const double tl_yaw = normalizeAngle(
        std::atan2(tl_right_down_point.y() - tl_left_down_point.y(), tl_right_down_point.x() - tl_left_down_point.x()) +
        M_PI_2);
...
bool MapBasedDetector::isInDistanceRange(const geometry_msgs::Point& tl_point, const geometry_msgs::Point& camera_point,
...
void MapBasedDetector::publishVisibleTrafficLights(
    ...
    const std::vector<lanelet::ConstLineString3d>& visible_traffic_lights, const ros::Publisher& pub) {
  ...
  for (const auto& traffic_light : visible_traffic_lights) {
    ...
    tf2::Transform tf_map2camera(
        tf2::Quaternion(camera_pose_stamped.pose.orientation.x, camera_pose_stamped.pose.orientation.y,
                        camera_pose_stamped.pose.orientation.z, camera_pose_stamped.pose.orientation.w),
        tf2::Vector3(camera_pose_stamped.pose.position.x, camera_pose_stamped.pose.position.y,
...
double MapBasedDetector::normalizeAngle(const double& angle) { return std::atan2(std::cos(angle), std::sin(angle)); }
