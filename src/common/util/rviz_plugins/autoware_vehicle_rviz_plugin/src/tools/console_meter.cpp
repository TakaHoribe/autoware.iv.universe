...
void ConsoleMeterDisplay::processMessage(const autoware_vehicle_msgs::VehicleStatusStampedConstPtr &msg_ptr)
{
  ...
  painter.drawLine(w / 2, h / 2, (w / 2) + ((double)w / 2.0) * std::cos(theta), (h / 2) + ((double)h / 2.0) * std::sin(theta));
  painter.setPen(QPen(white_color, int(2), Qt::SolidLine));
  painter.drawLine((w / 2) + ((double)w / 8.0) * std::cos(min_range_theta),
                   (h / 2) + ((double)h / 8.0) * std::sin(min_range_theta),
                   (w / 2) + ((double)w / 2.0) * std::cos(min_range_theta),
                   (h / 2) + ((double)h / 2.0) * std::sin(min_range_theta));
  painter.setPen(QPen(white_color, int(2), Qt::SolidLine));
  painter.drawLine((w / 2) + ((double)w / 8.0) * std::cos(max_range_theta),
                   (h / 2) + ((double)h / 8.0) * std::sin(max_range_theta),
                   (w / 2) + ((double)w / 2.0) * std::cos(max_range_theta),
                   (h / 2) + ((double)h / 2.0) * std::sin(max_range_theta));
  painter.drawArc(0, 0, w, h, 16 * ((min_range_theta - M_PI) * 180.0 / M_PI), 16 * ((max_range_theta - min_range_theta) * 180.0 / M_PI));
  painter.drawArc(w*3 / 8, h*3 / 8, w / 4, h / 4, 16 * ((min_range_theta - M_PI) * 180.0 / M_PI), 16 * ((max_range_theta - min_range_theta) * 180.0 / M_PI));
