...
std::unique_ptr<Ogre::ColourValue> ConsoleMeterDisplay::setColorDependsOnVelocity(const double vel_max,
...
ConsoleMeterDisplay::ConsoleMeterDisplay()
  ...
  property_text_color_ = new rviz::ColorProperty("Text Color", QColor(25, 255, 240), "text color", this,
                                                 SLOT(updateVisualization()), this);
  property_left_ =
      new rviz::IntProperty("Left", 128, "Left of the plotter window", this, SLOT(updateVisualization()), this);
  property_left_->setMin(0);
  property_top_ = new rviz::IntProperty("Top", 128, "Top of the plotter window", this, SLOT(updateVisualization()));
  property_top_->setMin(0);

  property_length_ =
      new rviz::IntProperty("Length", 256, "Length of the plotter window", this, SLOT(updateVisualization()), this);
  property_length_->setMin(10);
  property_value_height_offset_ = new rviz::IntProperty("Value height offset", 0, "Height offset of the plotter window",
                                                        this, SLOT(updateVisualization()));
  property_handle_angle_scale_ = new rviz::FloatProperty("Scale", 3.0, "Scale is steering andle to handle angle ", this,
                                                         SLOT(updateVisualization()), this);
...
void ConsoleMeterDisplay::processMessage(const geometry_msgs::TwistStampedConstPtr& msg_ptr) {
  ...
  const double velocity_ratio = std::min(std::max(std::fabs(msg_ptr->twist.linear.x) - meter_min_velocity_, 0.0) /
  ...
  painter.drawLine(w / 2, h / 2, (w / 2) + ((double)w / 2.0) * std::cos(theta),
                   (h / 2) + ((double)h / 2.0) * std::sin(theta));
  painter.setPen(QPen(white_color, int(2), Qt::SolidLine));
  painter.drawLine(
      (w / 2) + ((double)w / 8.0) * std::cos(min_range_theta), (h / 2) + ((double)h / 8.0) * std::sin(min_range_theta),
      (w / 2) + ((double)w / 2.0) * std::cos(min_range_theta), (h / 2) + ((double)h / 2.0) * std::sin(min_range_theta));
  painter.setPen(QPen(white_color, int(2), Qt::SolidLine));
  painter.drawLine(
      (w / 2) + ((double)w / 8.0) * std::cos(max_range_theta), (h / 2) + ((double)h / 8.0) * std::sin(max_range_theta),
      (w / 2) + ((double)w / 2.0) * std::cos(max_range_theta), (h / 2) + ((double)h / 2.0) * std::sin(max_range_theta));
  painter.drawArc(0, 0, w, h, 16 * ((min_range_theta - M_PI) * 180.0 / M_PI),
                  16 * ((max_range_theta - min_range_theta) * 180.0 / M_PI));
  painter.drawArc(w * 3 / 8, h * 3 / 8, w / 4, h / 4, 16 * ((min_range_theta - M_PI) * 180.0 / M_PI),
                  16 * ((max_range_theta - min_range_theta) * 180.0 / M_PI));
  ...
  painter.drawText(0, std::min(property_value_height_offset_->getInt(), h - 1), w,
