...
{
VelodyneDriver::VelodyneDriver(ros::NodeHandle node, ros::NodeHandle private_nh)
{
  ...
  {            // 1 packet holds 384 points
    ...
    packet_rate =
      754;  // 754 Packets/Second for Last or Strongest mode 1508 for dual (VLP-16 User Manual)
...
bool VelodyneDriver::poll(void)
{
  ...
  if (config_.cut_angle >= 0)  //Cut at specific angle feature enabled
  {
    ...
    while (true) {
      ...
      if (
        (last_azimuth < config_.cut_angle && config_.cut_angle <= azimuth) ||
