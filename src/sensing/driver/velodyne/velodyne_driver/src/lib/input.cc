...
/** @brief constructor
   ...
   */
InputPCAP::InputPCAP(
  ros::NodeHandle private_nh, uint16_t port, double packet_rate, std::string filename,
...
int InputPCAP::getPacket(velodyne_msgs::VelodynePacket * pkt, const double time_offset)
{
  ...
  while (true) {
    ...
    if ((res = pcap_next_ex(pcap_, &header, &pkt_data)) >= 0) {
      ...
      pkt->stamp =
        ros::Time::now();  // time_offset not considered here, as no synchronization required
      empty_ = false;
      return 0;  // success
    }
