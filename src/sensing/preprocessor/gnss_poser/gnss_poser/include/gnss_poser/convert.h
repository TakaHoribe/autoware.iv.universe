...
GNSSStat NavSatFix2UTM(const sensor_msgs::NavSatFix& nav_sat_fix_msg) {
  ...
  try {
    GeographicLib::UTMUPS::Forward(nav_sat_fix_msg.latitude, nav_sat_fix_msg.longitude, utm.zone, utm.northup, utm.x,
                                   utm.y);
...
GNSSStat UTM2MGRS(const GNSSStat& utm, const MGRSPrecision& precision) {
  ...
  try {
    ...
    GeographicLib::MGRS::Forward(utm.zone, utm.northup, utm.x, utm.y, utm.latitude, static_cast<int>(precision),
                                 mgrs_code);
