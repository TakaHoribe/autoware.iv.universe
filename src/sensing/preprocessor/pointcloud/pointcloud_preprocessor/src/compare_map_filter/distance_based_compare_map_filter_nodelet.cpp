...
void DistanceBasedCompareMapFilterNodelet::filter(const PointCloud2::ConstPtr& input, const IndicesPtr& indices,
  ...
  pcl::getPointCloudDifference<pcl::PointXYZ>(*pcl_input, *map_ptr_, distance_threshold_ * distance_threshold_, tree_,
                                              *pcl_output);
...
void DistanceBasedCompareMapFilterNodelet::config_callback(pointcloud_preprocessor::CompareMapFilterConfig& config,
                                                           uint32_t level) {
  boost::mutex::scoped_lock lock(mutex_);
