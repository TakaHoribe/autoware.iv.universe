...
void VoxelBasedCompareMapFilterNodelet::filter(const PointCloud2::ConstPtr& input, const IndicesPtr& indices,
  ...
  for (size_t i = 0; i < pcl_input->points.size(); ++i) {
    ...
    if (is_in_voxel(pcl::PointXYZ(point.x, point.y, point.z), point, distance_threshold_, voxel_map_ptr_, voxel_grid_))
      continue;
    if (is_in_voxel(pcl::PointXYZ(point.x, point.y - distance_threshold_, point.z - distance_threshold_), point,
    ...
    if (is_in_voxel(
            pcl::PointXYZ(point.x - distance_threshold_, point.y - distance_threshold_, point.z - distance_threshold_),
            point, distance_threshold_, voxel_map_ptr_, voxel_grid_))
      continue;
    if (is_in_voxel(pcl::PointXYZ(point.x - distance_threshold_, point.y - distance_threshold_, point.z), point,
                    distance_threshold_, voxel_map_ptr_, voxel_grid_))
      continue;
    if (is_in_voxel(
            pcl::PointXYZ(point.x - distance_threshold_, point.y - distance_threshold_, point.z + distance_threshold_),
            point, distance_threshold_, voxel_map_ptr_, voxel_grid_))
      continue;
    if (is_in_voxel(pcl::PointXYZ(point.x - distance_threshold_, point.y, point.z - distance_threshold_), point,
                    distance_threshold_, voxel_map_ptr_, voxel_grid_))
      continue;
...
bool VoxelBasedCompareMapFilterNodelet::is_in_voxel(const pcl::PointXYZ& src_point, const pcl::PointXYZ& target_point,
...
void VoxelBasedCompareMapFilterNodelet::config_callback(pointcloud_preprocessor::CompareMapFilterConfig& config,
                                                        uint32_t level) {
  boost::mutex::scoped_lock lock(mutex_);
...
PLUGINLIB_EXPORT_CLASS(pointcloud_preprocessor::VoxelBasedCompareMapFilterNodelet, nodelet::Nodelet);