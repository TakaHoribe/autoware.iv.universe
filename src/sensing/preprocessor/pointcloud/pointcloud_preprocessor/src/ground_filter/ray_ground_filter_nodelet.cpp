...
void RayGroundFilterNodelet::ExtractPointsIndices(const pcl::PointCloud<PointType_>::Ptr in_cloud_ptr,
...
void RayGroundFilterNodelet::filter(const PointCloud2::ConstPtr& input, const IndicesPtr& indices,
                                    PointCloud2& output) {
  ...
  if (!succeeded) {
    ROS_ERROR_STREAM_THROTTLE(10, "Failed transform from " << base_frame_ << " to " << input->header.frame_id);
    return;
  }
  ...
  ConvertXYZIToRTZColor(current_sensor_cloud_ptr, organized_points, radial_division_indices, radial_ordered_clouds);
  ...
  if (!succeeded) {
    ROS_ERROR_STREAM_THROTTLE(
        10, "Failed transform from " << base_frame_ << " to " << no_ground_cloud_msg_ptr->header.frame_id);
    return;
  }
...
void RayGroundFilterNodelet::config_callback(pointcloud_preprocessor::RayGroundFilterConfig& config, uint32_t level) {
  boost::mutex::scoped_lock lock(mutex_);

  if (base_frame_ != config.base_frame) {
    base_frame_ = config.base_frame;
    NODELET_DEBUG("[%s::config_callback] Setting base_frame to: %s.", getName().c_str(), config.base_frame.c_str());
  }
