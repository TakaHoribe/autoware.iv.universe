...
//  config_sub_ = nh_.subscribe("/config/ndt_slam", 1, &NdtSlam::configCallback, this);
  ...
  f_ = boost::bind(&NdtSlam::configCallback, this, _1, _2);
...
void NdtSlam::configCallback(const lidar_localizer::NdtSlamConfig &config, uint32_t level)
{
  init_pose_stamped_.pose = Pose(config.init_x, config.init_y,
                                 ...
                                 config.init_pitch, config.init_yaw);
  pose_interpolator_.clearPoseStamped();

  localizer_ptr_->setStepSize(config.step_size);
  localizer_ptr_->setTransformationEpsilon(config.trans_epsilon);
  localizer_ptr_->setMaximumIterations(config.max_iterations);
  localizer_ptr_->setResolution(config.resolution);

  if (config.with_mapping == true && with_mapping_ == false) {
    if (separate_mapping_) {
      map_manager_.saveSeparateMap();
      map_manager_.loadAroundMap(localizer_ptr_->getLocalizerPose());
    }
  }
  else if (config.with_mapping == false && with_mapping_ == true) {
    if (separate_mapping_) {
      map_manager_.saveSeparateMapThread();
    }
    else {
  ...
  with_mapping_ = config.with_mapping;
}
