...
void RequestQueue::clear_look_ahead() {
  std::unique_lock<std::mutex> lock(mtx_);
  while (!look_ahead_queue_.empty()) look_ahead_queue_.pop();
}

geometry_msgs::Point RequestQueue::dequeue() {
  std::unique_lock<std::mutex> lock(mtx_);
  while (queue_.empty() && look_ahead_queue_.empty()) cv_.wait(lock);
  if (!queue_.empty()) {
  ...
  while (true) {
    ...
    for (const std::string& loc : locs) {  // XXX better way?
      ...
      if (is_downloaded(arealist_path))
        ...
        if (download(gf, TEMPORARY_DIRNAME, loc, AREALIST_FILENAME) != 0) continue;
        areas = read_arealist(arealist_path);
        for (Area& area : areas) area.path = TEMPORARY_DIRNAME + loc + basename(area.path.c_str());
        write_arealist(arealist_path, areas);
      }
      for (const Area& area : areas) cache_arealist(area, all_areas);
...
void publish_gnss_pcd(const geometry_msgs::PoseStamped& msg) {
  ...
  if (((now - current_time).toSec() * 1000) < fallback_rate) return;
  if (((now - gnss_time).toSec() * 1000) < update_rate) return;
  gnss_time = now;

  if (can_download) request_queue.enqueue(msg.pose.position);
...
void publish_current_pcd(const geometry_msgs::PoseStamped& msg) {
  ros::Time now = ros::Time::now();
  if (((now - current_time).toSec() * 1000) < update_rate) return;
  current_time = now;
...
void publish_dragged_pcd(const geometry_msgs::PoseWithCovarianceStamped& msg) {
  ...
  if (can_download) request_queue.enqueue(p);
...
int main(int argc, char** argv) {
  ...
  if (margin < 0) {
    ...
    if (can_download) {
      ...
      for (const Area& area : areas) {
        for (const std::string& path : pcd_paths) {
          if (path == area.path) cache_arealist(area, downloaded_areas);
        }
      }
    }
