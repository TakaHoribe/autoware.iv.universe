...
void addMarkers(const std::unique_ptr<GpkgInterface>& gpkg_interface, MarkerArray* marker_array,
                const std_msgs::ColorRGBA& color, const std::function<void(const T&, Marker*)> post_process = nullptr,
  ...
  for (const auto& feature : features) {
    auto marker = createMarker("map", ns, static_cast<int32_t>(feature.id), feature.geometry, color);
...
MarkerArray createAllMarkers(const std::unique_ptr<GpkgInterface>& gpkg_interface) {
  ...
  addMarkers<avm::IntersectionArea>(gpkg_interface, &marker_array, createMarkerColor(0.7, 0.0, 0.0, 0.5));

  addMarkers<avm::LaneSection>(gpkg_interface, &marker_array, createMarkerColor(0.0, 0.7, 0.0, 0.5));

  addMarkers<avm::Lane>(gpkg_interface, &marker_array, createMarkerColor(0.2, 0.7, 0.7, 0.5),
                        [](const auto& f, Marker* marker) { marker->scale = createMarkerScale(f.width, 0.0, 0.0); },
                        "LaneArea");

  addMarkers<avm::Lane>(gpkg_interface, &marker_array, createMarkerColor(0.0, 0.0, 0.0, 0.999),
                        [](const auto& f, Marker* marker) { marker->scale = createMarkerScale(0.5, 0.0, 0.0); },
                        "LaneCenterLine");

  addMarkers<avm::StopLine>(gpkg_interface, &marker_array, createMarkerColor(1.0, 0.0, 0.0, 0.999),
                            [](const auto& f, Marker* marker) { marker->scale = createMarkerScale(0.5, 0.0, 0.0); });
...
VectorMapVisualizerNode::VectorMapVisualizerNode() : nh_(""), private_nh_("~") {
  ...
  sub_binary_gpkg_map_ =
      nh_.subscribe("autoware_vector_map/binary_gpkg_map", 1, &VectorMapVisualizerNode::onBinaryGpkgMap, this);
}
