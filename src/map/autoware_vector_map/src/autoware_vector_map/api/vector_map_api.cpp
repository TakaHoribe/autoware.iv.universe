...
std::vector<ConstPtr<Lane>> VectorMapApi::findNearLanes(const Point3d& p, const double range) {
  return gpkg_interface_->findFeaturesByRange<Lane>(p, range);
}

ConstPtr<Lane> VectorMapApi::findNearestLane(const std::vector<ConstPtr<Lane>>& lanes,
                                             const Point3d& p) {
...
ConstPtr<Lane> VectorMapApi::findNearestLane(const Point3d& p, const double range) {
...
std::vector<ConstPtr<Lane>> VectorMapApi::getNextLanes(const Lane& lane) {
  return gpkg_interface_->getRelatedFeaturesById<LaneConnection, RelationSide::Left>(lane.id);
}

std::vector<ConstPtr<Lane>> VectorMapApi::getPrevLanes(const Lane& lane) {
  return gpkg_interface_->getRelatedFeaturesById<LaneConnection, RelationSide::Right>(lane.id);
}

std::vector<ConstPtr<StopLine>> VectorMapApi::getRelatedStopLines(const Lane& lane) {
  return gpkg_interface_->getRelatedFeaturesById<Lane_StopLine, RelationSide::Left>(lane.id);
}

std::vector<ConstPtr<Crosswalk>> VectorMapApi::getRelatedCrosswalks(const Lane& lane) {
