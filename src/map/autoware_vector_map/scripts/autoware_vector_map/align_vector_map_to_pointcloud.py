...
def find_nearest_z_value(points, x, y, z):
    ...
    new_z = np.min(filtered_points[:, 2])
...
def create_new_geometry(points, geometry):
    ...
    if geometry.geom_type == "LineString":
        ...
        for coord in geometry.coords:
            new_coords.append(create_new_coord(filtered_points, coord))
    ...
    if geometry.geom_type == "Polygon":
        new_exterior = create_new_geometry(filtered_points, geometry.exterior).coords

        new_interiors = []
        for interior in geometry.interiors:
            new_interiors.append(create_new_geometry(filtered_points, interior).coords)
...
def align_features(map_api, table_name, points):
    ...
    for i, row in gdf.iterrows():
        gdf.loc[i, "geometry"] = create_new_geometry(filtered_points, row.geometry)
