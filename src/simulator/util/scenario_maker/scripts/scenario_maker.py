...
class ScenarioMaker():
    ...
    def scenario_obstacle_manager1(self):
        #obstacle 1
        if self.obstacle_generate_judge_dist(-56.7, 57.3, 27.2, 15.0, 50.0):
            if self.obstacle_generated[1] == 0:
                self.PubObstacle(\
                                pose = self.random_pose_maker(x=-56.7, y=57.3, th=27.2, ver_sigma=1.0, lat_sigma=0.0, th_sigma=0.0),\
                                vel = self.random_velocity_maker(v=4.0, v_sigma=0.1), \
                                obstacle_type="car", obstacle_id=1)
                self.obstacle_generated[1] = 1

        #obstacle 2
        if self.obstacle_generate_judge_dist(-95.0, -67.0, -152.8, 15.0, 50.0):
    ...
    def reset_obstacle(self):
        self.obstacle_generated = np.zeros((OBSTACLE_NUM))
        pass#TODO (pub reset)
    ...
    def obstacle_generate_judge_dist(self, x, y, th, dist_area, th_dist_area):
        ...
        if now_dist < dist_area and th < th_dist_area:
            return True
        else:
            return False
