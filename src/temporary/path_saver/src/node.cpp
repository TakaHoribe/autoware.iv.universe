...
PathSaverNode::PathSaverNode() : nh_(), pnh_("~"), tf_listener_(tf_buffer_)
{
    ...
    ofs_ << "x, y, z, roll[rad], pitch[rad], yaw[rad], linear_velocity[m/s], angular_velocity[rad/s]"
         << std::endl;
}
...
void PathSaverNode::timerCallback(const ros::TimerEvent &)
{
    ...
    if (last_saved_time_ == nullptr){
        ...
        if (dist < dist_threshold_ && dist_from_velocity < dist_threshold_)
            return;
    }
    ...
    ofs_ << self_pose.transform.translation.x << ", "
         << self_pose.transform.translation.y << ", "
         << self_pose.transform.translation.z << ", "
         << roll << ", "
         << pitch << ", "
         << yaw << ", "
