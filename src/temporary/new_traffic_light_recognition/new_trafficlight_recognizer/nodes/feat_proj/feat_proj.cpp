 ...
 */
bool project2(const Point3 &pt, Point3 &transformed_pt, int &u, int &v, bool useOpenGLCoord = false)
{
    float nearPlane = 1.0;
...
void echoSignals2(ros::Publisher &pub, ros::Publisher &pub_proj, bool useOpenGLCoord = false)
{
    int countPoint = 0;
    autoware_msgs::Signals signalsInFrame;
    autoware_msgs::Signals signalsInFrameProj;
    ...
    for (const auto& signal_map : vmap.signals)
	{
            ...
            int pid = vmap.vectors[signal.vid].pid; // corresponded point id

            Point3 signalcenter = vmap.getPoint(pid);
            Point3 signalcenterx(signalcenter.x(), signalcenter.y(), signalcenter.z() + SignalLampRadius);

            int u, v;
            Point3 transformed_pt;
            if (project2(signalcenter, transformed_pt, u, v, useOpenGLCoord) == true)
		{
                    countPoint++;
                    ...
                    int ux, vx;
                    Point3 pt;
                    project2(signalcenterx, pt, ux, vx, useOpenGLCoord);
			...
			{
                            // std::cout << u << ", " << v << ", " << transformed_pt.z() << std::endl;
                            signalsInFrame.Signals.push_back(sign);


                            autoware_msgs::ExtractedPosition sign_proj;
                            sign_proj = sign;
                            sign_proj.x = transformed_pt.x();
                            sign_proj.y = transformed_pt.y();
                            sign_proj.z = transformed_pt.z();
                            signalsInFrameProj.Signals.push_back(sign_proj);
			}
		}
	}
    signalsInFrame.header = header_;
    signalsInFrameProj.header = header_;
    pub.publish(signalsInFrame);
    pub_proj.publish(signalsInFrameProj);
...
int main(int argc, char *argv[])
{
    ...
    ros::Publisher signalPublisher2 = rosnode.advertise<autoware_msgs::Signals>("roi_signal_proj", 100);
    ...
    while (true)
	{
		...
		{
                    echoSignals2(signalPublisher, signalPublisher2, false);
		}
