...
int main(int argc, char* argv[]) {
  ...
  u16 duration = 0;
  rq1_imu_diagnostic_device_status_field imu_diagnostic_field;
  rq1_imu_basic_status_field imu_basic_field;
  rq1_diagnostic_device_status_field diagnostic_field;
  rq1_basic_status_field basic_field;
  ...
  com_mode = MIP_SDK_RQ1_IMU_DIRECT_MODE;
  ...
  if (com_mode == MIP_SDK_RQ1_IMU_DIRECT_MODE) {
    ...
    while (mip_3dm_cmd_hw_specific_imu_device_status(&device_interface, RQ1_IMU_MODEL_NUMBER, RQ1_IMU_BASIC_STATUS_SEL,
                                                     (u8*)&imu_basic_field) != MIP_INTERFACE_OK) {
    }

    printf("Model Number: \t\t\t\t\t%04u\n", imu_basic_field.device_model);
    printf("Status Selector: \t\t\t\t%s\n", imu_basic_field.status_selector == RQ1_IMU_BASIC_STATUS_SEL
                                                ? "Basic Status Report"
                                                : "Diagnostic Status Report");
    printf("Status Flags: \t\t\t\t\t0x%08x\n", imu_basic_field.status_flags);
    printf("System Microsecond Timer Count: \t\t%llu us\n\n", imu_basic_field.system_timer_us);

    printf("Requesting DIAGNOSTIC Status Report:\n");

    // Request diagnostic status report
    while (mip_3dm_cmd_hw_specific_imu_device_status(&device_interface, RQ1_IMU_MODEL_NUMBER,
                                                     RQ1_IMU_DIAGNOSTICS_STATUS_SEL,
                                                     (u8*)&imu_diagnostic_field) != MIP_INTERFACE_OK) {
    }

    printf("Model Number: \t\t\t\t\t%04u\n", imu_diagnostic_field.device_model);
    printf("Status Selector: \t\t\t\t%s\n", imu_diagnostic_field.status_selector == RQ1_IMU_BASIC_STATUS_SEL
                                                ? "Basic Status Report"
                                                : "Diagnostic Status Report");
    printf("Status Flags: \t\t\t\t\t0x%08x\n", imu_diagnostic_field.status_flags);
    printf("System Microsecond Timer Count: \t\t%llu us\n", imu_diagnostic_field.system_timer_us);
    ...
    printf("Bad Temperature Sensor Detected: \t\t%s\n",
           imu_diagnostic_field.bad_temp_sensor_detected == 1 ? "TRUE" : "FALSE");
    printf("Cold Temperature Detected: \t\t\t%s\n", imu_diagnostic_field.cold_temp_detected == 1 ? "TRUE" : "FALSE");
    printf("Bad Internal Voltage Detected: \t\t\t%s\n",
           imu_diagnostic_field.bad_voltage_detected == 1 ? "TRUE" : "FALSE");
    ...
    printf("Communications Port State: \t\t\t%s\n",
           imu_diagnostic_field.com_port_state == 1 ? "Initialized" : "Uninitialized");
    printf("Communications Port Internal Baud: \t\t%u Bd\n", imu_diagnostic_field.com_port_baudrate);
  ...
  com_mode = MIP_SDK_RQ1_IMU_STANDARD_MODE;
  ...
  if (com_mode == MIP_SDK_RQ1_IMU_STANDARD_MODE) {
    ////////////////////////////////////////////////////////////////////////////////////
    //
    // Base Command Tests
    //
    ...
    // Request basic status report
    while (mip_3dm_cmd_hw_specific_device_status(&device_interface, RQ1_MODEL_NUMBER, RQ1_BASIC_STATUS_SEL,
                                                 (u8*)&basic_field) != MIP_INTERFACE_OK) {
    }

    printf("Model Number: \t\t\t\t\t%04u\n", basic_field.device_model);
    printf("Status Selector: \t\t\t\t%s\n", basic_field.status_selector == RQ1_IMU_BASIC_STATUS_SEL
                                                ? "Basic Status Report"
                                                : "Diagnostic Status Report");
    printf("Status Flags: \t\t\t\t\t0x%08x\n", basic_field.status_flags);

    if (basic_field.system_state == RQ1_SYSTEM_STATE_INITIALIZATION) {
      strcpy(temp_string, "System Initialization");
    } else if (basic_field.system_state == RQ1_SYSTEM_STATE_SENSOR_STARTUP) {
      strcpy(temp_string, "Sensor Start-up");
    } else if (basic_field.system_state == RQ1_SYSTEM_STATE_RUNNING) {
    ...
    // Request diagnostic status report
    while (mip_3dm_cmd_hw_specific_device_status(&device_interface, RQ1_MODEL_NUMBER, RQ1_DIAGNOSTICS_STATUS_SEL,
                                                 (u8*)&diagnostic_field) != MIP_INTERFACE_OK) {
    }

    printf("Model Number: \t\t\t\t\t%04u\n", diagnostic_field.device_model);
    printf("Status Selector: \t\t\t\t%s\n", diagnostic_field.status_selector == RQ1_IMU_BASIC_STATUS_SEL
                                                ? "Basic Status Report"
                                                : "Diagnostic Status Report");
    printf("Status Flags: \t\t\t\t\t0x%08x\n", diagnostic_field.status_flags);
    printf("System Millisecond Timer Count: \t\t%llu ms\n", diagnostic_field.system_timer_ms);
    printf("Measured Internal Temperature: \t\t\t%f degrees C\n", diagnostic_field.temp_degc);
    printf("Last Temperature Measured: \t\t\t%u ms\n", diagnostic_field.last_temp_read_ms);
    printf("Cold Temperature Detected: \t\t\t%s\n", diagnostic_field.cold_condition == 1 ? "TRUE" : "FALSE");
...
void print_command_line_usage() {
  ...
  printf("   RQ1_Test [com_port_num] [baudrate]\n");
  printf("\n\n");
  printf("   Example: \"RQ1_Test 1 115200\", Opens a connection to the \n");
  printf("             RQ1 on COM1, with a baudrate of 115200.\n");
  printf("\n\n");
...
//
//! @section DESCRIPTION
//! Requests RQ1-IMU Basic or Diagnostic Status Message.
//
//! @section DETAILS
//!
//! @param [in] mip_interface *device_interface - pointer to the mip interface structure.
//! @param [in] u16 model_number - LORD Microstrain Sensing Systems model number for RQ1 IMU (6238)
...
u16 mip_3dm_cmd_hw_specific_imu_device_status(mip_interface* device_interface, u16 model_number, u8 status_selector,
                                              u8* response_buffer) {
  rq1_imu_basic_status_field* basic_ptr;
  rq1_imu_diagnostic_device_status_field* diagnostic_ptr;
  u16 response_size = MIP_FIELD_HEADER_SIZE;

  if (status_selector == RQ1_BASIC_STATUS_SEL)
    response_size += sizeof(rq1_imu_basic_status_field);
  else if (status_selector == RQ1_DIAGNOSTICS_STATUS_SEL)
    response_size += sizeof(rq1_imu_diagnostic_device_status_field);

  while (mip_3dm_cmd_device_status(device_interface, model_number, status_selector, response_buffer, &response_size) !=
         MIP_INTERFACE_OK) {
  }

  if (status_selector == RQ1_IMU_BASIC_STATUS_SEL) {
    if (response_size != sizeof(rq1_imu_basic_status_field))
      return MIP_INTERFACE_ERROR;
    else if (MIP_SDK_CONFIG_BYTESWAP) {
      basic_ptr = (rq1_imu_basic_status_field*)response_buffer;
      ...
      byteswap_inplace(&basic_ptr->system_timer_us, sizeof(basic_ptr->system_timer_us));
    }

  } else if (status_selector == RQ1_IMU_DIAGNOSTICS_STATUS_SEL) {
    if (response_size != sizeof(rq1_imu_diagnostic_device_status_field))
      return MIP_INTERFACE_ERROR;
    else if (MIP_SDK_CONFIG_BYTESWAP) {
      diagnostic_ptr = (rq1_imu_diagnostic_device_status_field*)response_buffer;
      ...
      byteswap_inplace(&diagnostic_ptr->system_timer_us, sizeof(diagnostic_ptr->system_timer_us));
      ...
      byteswap_inplace(&diagnostic_ptr->com_port_state, sizeof(diagnostic_ptr->com_port_state));
      byteswap_inplace(&diagnostic_ptr->com_port_baudrate, sizeof(diagnostic_ptr->com_port_baudrate));
...
//
//! @section DESCRIPTION
//! Requests RQ1 Basic or Diagnostic Status Message.
//
//! @section DETAILS
//!
//! @param [in] mip_interface *device_interface - pointer to the mip interface structure.
...
u16 mip_3dm_cmd_hw_specific_device_status(mip_interface* device_interface, u16 model_number, u8 status_selector,
                                          u8* response_buffer) {
  rq1_basic_status_field* basic_ptr;
  rq1_diagnostic_device_status_field* diagnostic_ptr;
  u16 response_size = MIP_FIELD_HEADER_SIZE;

  if (status_selector == RQ1_BASIC_STATUS_SEL)
    response_size += sizeof(rq1_basic_status_field);
  else if (status_selector == RQ1_DIAGNOSTICS_STATUS_SEL)
    response_size += sizeof(rq1_diagnostic_device_status_field);
  ...
  if (status_selector == RQ1_BASIC_STATUS_SEL) {
    if (response_size != sizeof(rq1_basic_status_field))
      return MIP_INTERFACE_ERROR;
    else if (MIP_SDK_CONFIG_BYTESWAP) {
      basic_ptr = (rq1_basic_status_field*)response_buffer;
    ...
    if (response_size != sizeof(rq1_diagnostic_device_status_field))
      return MIP_INTERFACE_ERROR;
    else if (MIP_SDK_CONFIG_BYTESWAP) {
      diagnostic_ptr = (rq1_diagnostic_device_status_field*)response_buffer;
