...
int main(int argc, char* argv[]) {
  ...
  if (com_mode == MIP_SDK_GX4_25_IMU_STANDARD_MODE) {
    ...
    // Request basic status report
    while (mip_3dm_cmd_hw_specific_device_status(&device_interface, GX4_25_MODEL_NUMBER, GX4_25_BASIC_STATUS_SEL,
    ...
    // Request diagnostic status report
    while (mip_3dm_cmd_hw_specific_device_status(&device_interface, GX4_25_MODEL_NUMBER, GX4_25_DIAGNOSTICS_STATUS_SEL,
                                                 &diagnostic_field) != MIP_INTERFACE_OK) {
    }

    printf("Model Number: \t\t\t\t\t%04u\n", diagnostic_field.device_model);
    printf("Status Selector: \t\t\t\t%s\n", diagnostic_field.status_selector == GX4_25_BASIC_STATUS_SEL
                                                ? "Basic Status Report"
                                                : "Diagnostic Status Report");
    ...
    external_heading_update.type = 0x2;                   // Magnetic Heading
    ...
    for (i = 3; i >= 0; i--) {
      ...
      // Skip GPS velocity as a heading source
      if (heading_source != 2) {
