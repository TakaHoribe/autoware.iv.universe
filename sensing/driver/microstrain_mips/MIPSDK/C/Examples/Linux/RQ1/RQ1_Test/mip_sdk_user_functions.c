...
s32 fd;
s32 log_bytes_written, log_bytes_to_write;
time_t t;
struct tm tm;
u8 log_message_buffer[LOG_MESSAGE_BUFFER_SIZE] = {0};
u8 byte_str[3] = {0};
u32 byte_index;
...
u16 mip_sdk_port_open(void** port_handle, int port_num, int baudrate) {
  ...
  *port_handle = &local_port_handle;
  ...
  // Create Log file or append existing log file
  if (access(MIP_COMMUNICATION_LOG_FILENAME, F_OK) != -1)
    fd = open(MIP_COMMUNICATION_LOG_FILENAME, O_WRONLY | O_APPEND);
  else
    fd = open(MIP_COMMUNICATION_LOG_FILENAME, O_WRONLY | O_CREAT);

  t = time(NULL);

  // update time to start logging
  tm = *localtime(&t);

  log_bytes_to_write = sprintf(log_message_buffer, "\nBEGINNING LOG %d-%d-%d %d:%d:%d\n", tm.tm_year + 1900,
                               tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);

  if (log_bytes_to_write < 0 || write(fd, log_message_buffer, log_bytes_to_write) != log_bytes_to_write)
...
u16 mip_sdk_port_write(void* port_handle, u8* buffer, u32 num_bytes, u32* bytes_written, u32 timeout_ms) {
  ...
  if (local_bytes_written == -1) {
    printf("Unable to write to com Port %s\n Errno = %i\n", port_name, errno);
  ...
  *bytes_written = local_bytes_written;
  if (*bytes_written == num_bytes) {
    ...
    // Begin Log Message
    log_bytes_to_write =
        sprintf(log_message_buffer, "%d:%d:%d Writing Bytes %i bytes:\n", tm.tm_hour, tm.tm_min, tm.tm_sec, num_bytes);
    ...
    for (byte_index = 0; byte_index < num_bytes; byte_index++) {
      snprintf(byte_str, 3, "%.2X\0", buffer[byte_index]);
      strcat(log_message_buffer, byte_str);
    }
    strcat(log_message_buffer, "\n");

    log_bytes_to_write = strlen(log_message_buffer);
...
u16 mip_sdk_port_read(void* port_handle, u8* buffer, u32 num_bytes, u32* bytes_read, u32 timeout_ms) {
  ...
  if (local_bytes_read == -1) {
    printf("Unable to read_from com Port %s\n Errno = %i\n", port_name, errno);
  ...
  *bytes_read = local_bytes_read;
  if (*bytes_read == num_bytes) {
    ...
    // Begin Log Message
    log_bytes_to_write =
        sprintf(log_message_buffer, "%d:%d:%d Reading Bytes %i bytes:\n", tm.tm_hour, tm.tm_min, tm.tm_sec, num_bytes);
