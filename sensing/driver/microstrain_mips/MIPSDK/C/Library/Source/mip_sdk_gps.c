...
void mip_gps_ecef_pos_byteswap(mip_gps_ecef_pos* ecef_pos) {
  ...
  for (i = 0; i < 3; i++) byteswap_inplace(&ecef_pos->x[i], sizeof(double));
...
void mip_gps_ned_vel_byteswap(mip_gps_ned_vel* ned_vel) {
  ...
  for (i = 0; i < 3; i++) byteswap_inplace(&ned_vel->v[i], sizeof(float));
...
void mip_gps_ecef_vel_byteswap(mip_gps_ecef_vel* ecef_vel) {
  ...
  for (i = 0; i < 3; i++) byteswap_inplace(&ecef_vel->v[i], sizeof(float));
