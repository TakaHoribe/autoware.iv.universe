...
u16 mip_3dm_cmd_poll_ahrs(mip_interface* device_interface, u8 option_selector, u8 num_descriptors,
                          u8* descriptor_list) {
  ...
  if (num_descriptors >= MIP_MAX_PAYLOAD_SIZE) return MIP_INTERFACE_ERROR;
  ...
  if (option_selector == MIP_3DM_POLLING_ENABLE_ACK_NACK) wait_for_response = 1;

  // Send the command
  return mip_interface_send_command(device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_POLL_AHRS_MESSAGE, command_data,
                                    sizeof(u8) * 2 + num_descriptors * 3, wait_for_response,
...
u16 mip_3dm_cmd_poll_gps(mip_interface* device_interface, u8 option_selector, u8 num_descriptors, u8* descriptor_list) {
  ...
  // Send the command
  return mip_interface_send_command(device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_POLL_GPS_MESSAGE, command_data,
                                    sizeof(u8) * 2 + num_descriptors * 3, wait_for_response,
                                    MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
}
...
u16 mip_3dm_cmd_poll_filter(mip_interface* device_interface, u8 option_selector, u8 num_descriptors,
                            u8* descriptor_list) {
  u8 command_data[MIP_MAX_PAYLOAD_SIZE] = {0};
  u8 wait_for_response = 0;
  u8 i;
  ...
  // Send the command
  return mip_interface_send_command(device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_POLL_NAV_MESSAGE, command_data,
                                    sizeof(u8) * 2 + num_descriptors * 3, wait_for_response,
                                    MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
}
...
u16 mip_3dm_cmd_get_ahrs_base_rate(mip_interface* device_interface, u16* base_rate) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_GET_AHRS_BASE_RATE, NULL, 0, &response_data,
...
u16 mip_3dm_cmd_get_gps_base_rate(mip_interface* device_interface, u16* base_rate) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_GET_GPS_BASE_RATE, NULL, 0, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
u16 mip_3dm_cmd_get_filter_base_rate(mip_interface* device_interface, u16* base_rate) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_GET_NAV_BASE_RATE, NULL, 0, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//! @param [in,out] u8 *num_entries             - The number of descriptors provided or the number read back from the
//! device. (used to set or get depending on \c function_selector)
//! @param [in,out] u8 *descriptors             - A descriptor list containing \c num_descriptors descriptor values.
//! (used to set or get depending on \c function_selector)
//! @param [in,out] u8 *decimation              - A decimation list containing \c num_descriptors decimation values.
//! (used to set or get depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_3dm_cmd_ahrs_message_format(mip_interface* device_interface, u8 function_selector, u8* num_entries,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_AHRS_MESSAGE_FORMAT, command_data, command_data_size,
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] u8 *num_entries             - The number of descriptors provided/available or the number read back
//! from the device. (used to set or get depending on \c function_selector)
//! @param [in,out] u8 *descriptors             - A descriptor list containing \c num_descriptors descriptor values.
//! (used to set or get depending on \c function_selector)
//! @param [in,out] u8 *decimation              - A decimation list containing \c num_descriptors decimation values.
//! (used to set or get depending on \c function_selector)
...
u16 mip_3dm_cmd_gps_message_format(mip_interface* device_interface, u8 function_selector, u8* num_entries,
                                   u8* descriptors, u16* decimation) {
  u8 i;
  u8* response_data;
  u8 local_num_entries;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_GPS_MESSAGE_FORMAT, command_data, command_data_size,
      &response_data, &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
u16 mip_3dm_cmd_filter_message_format(mip_interface* device_interface, u8 function_selector, u8* num_entries,
                                      u8* descriptors, u16* decimation) {
  u8 i;
  u8* response_data;
  u8 local_num_entries;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_NAV_MESSAGE_FORMAT, command_data, command_data_size,
      &response_data, &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//! @param [in,out] u8 *enable                  - the enable/disable flag. (used to set or get depending on \c
//! function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_3dm_cmd_continuous_data_stream(mip_interface* device_interface, u8 function_selector, u8 device_selector,
                                       u8* enable) {
  u8* response_data;
  u16 response_data_size;
  u16 return_code;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_CONTROL_DATA_STREAM, command_data, 3, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] u8 *dynamics_mode           - The dynamics mode. (used to set or get depending on \c
//! function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_3dm_cmd_gps_dynamics_mode(mip_interface* device_interface, u8 function_selector, u8* dynamics_mode) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_SET_GPS_DYNAMICS_MODE, command_data, 2, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//! @param [in] mip_interface *device_interface        - The device interface.
//! @param [in] u8 function_selector                   - Selects which function to perform.
//! @param [in,out] mip_ahrs_signal_settings *settings - The AHRS signal conditioning settings structure. (used to set
//! or get depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
//! @retval MIP_INTERFACE_OK     The command was successful.\n
...
u16 mip_3dm_cmd_ahrs_signal_conditioning(mip_interface* device_interface, u8 function_selector,
                                         mip_ahrs_signal_settings* settings) {
  u8* response_data;
  u16 response_data_size;
  u16 return_code;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_SET_AHRS_SIGNAL_COND, command_data,
...
//! @param [in,out] u32 *time                   - The current time value. (used to set or get depending on \c
//! function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_3dm_cmd_ahrs_timestamp(mip_interface* device_interface, u8 function_selector, u8* time_selector, u32* time) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_SET_AHRS_TIMESTAMP, command_data, 6, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] u32 *baudrate               - The baudrate value. (used to set or get depending on \c
//! function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_3dm_cmd_uart_baudrate(mip_interface* device_interface, u8 function_selector, u32* baudrate) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_UART_BAUDRATE, command_data, 5, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_3dm_cmd_datastream_format(mip_interface *device_interface, u8 function_selector, u8 device_selector, u8
//! *format)
//
//! @section DESCRIPTION
...
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in] u8 device_selector              - Selects which device is affected.
//! @param [in,out] u8 *format                  - The selected format of the data. (used to set or get depending on \c
//! function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_3dm_cmd_datastream_format(mip_interface* device_interface, u8 function_selector, u8 device_selector,
                                  u8* format) {
  u8* response_data;
  u16 response_data_size;
  u16 return_code;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_DATASTREAM_FORMAT, command_data, 3, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_3dm_cmd_power_state(mip_interface *device_interface, u8 function_selector, u8 device_selector, u8
//! *power_state)
//
//! @section DESCRIPTION
...
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in] u8 device_selector              - Selects which device is affected.
//! @param [in,out] u8 *power_state             - The selected power state. (used to set or get depending on \c
//! function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_3dm_cmd_power_state(mip_interface* device_interface, u8 function_selector, u8 device_selector,
                            u8* power_state) {
  u8* response_data;
  u16 response_data_size;
  u16 return_code;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_DEVICE_POWER_STATE, command_data, 3, &response_data,
      &response_data_size, MIP_INTERFACE_LONG_COMMAND_RESPONSE_TIMEOUT_MS);
...
//! @param [in,out] u16 *response_size          - On entry, the size of the buffer.  On exit, the size of the returned
//! status message.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_3dm_cmd_device_status(mip_interface* device_interface, u16 model_number, u8 status_selector,
  ...
  *short_ptr = model_number;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_DEVICE_STATUS, command_data, 3, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->size - sizeof(mip_field_header)) <= user_buffer_size) {
      memcpy(response_buffer, response_data + sizeof(mip_field_header),
             field_header_ptr->size - sizeof(mip_field_header));
...
//
//! @fn
//! u16 mip_3dm_cmd_low_pass_filter_settings(mip_interface *device_interface, u8 function_selector,
...
//! @param [in,out] mip_low_pass_filter_settings *filter_settings - structure specifying the data type to be filtered
...
u16 mip_3dm_cmd_low_pass_filter_settings(mip_interface* device_interface, u8 function_selector,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_LOW_PASS_FILTER_SETTINGS, command_data,
...
//! @param [in] mip_interface *device_interface	- The device interface.
//! @param [in] u8 function_selector            - The model number of the device.
//! @param [in,out] float *bias_vector		- pointer to array containing input bias vector on write commands or to
//! contain output bias vector on read commands.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
//! @retval MIP_INTERFACE_OK     The command was successful.\n
...
u16 mip_3dm_cmd_accel_bias(mip_interface* device_interface, u8 function_selector, float* bias_vector) {
  ...
  if (function_selector == MIP_FUNCTION_SELECTOR_WRITE) {
    ...
    // Byteswap if enabled
    if (MIP_SDK_CONFIG_BYTESWAP) {
      for (i = 0; i < 3; i++) byteswap_inplace(&bias_element_pointer[i], sizeof(float));
    }
  }

  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_ACCEL_BIAS, command_data, sizeof(u8) + 3 * sizeof(float),
      &response_data, &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_3DM_REPLY_ACCEL_BIAS_VECTOR) &&
      ...
      // Byteswap the baudrate if enabled
      if (MIP_SDK_CONFIG_BYTESWAP) {
        for (i = 0; i < 3; i++) byteswap_inplace(&bias_vector[i], sizeof(float));
      }
    } else
      return_code = MIP_INTERFACE_ERROR;
  }
...
u16 mip_3dm_cmd_gyro_bias(mip_interface* device_interface, u8 function_selector, float* bias_vector) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_GYRO_BIAS, command_data, sizeof(u8) + 3 * sizeof(float),
      &response_data, &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
u16 mip_3dm_cmd_coning_sculling_compensation(mip_interface* device_interface, u8 function_selector, u8* enable) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CDM_CONING_AND_SCULLING_ENABLE, command_data, 2, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//! @param [in] mip_interface *device_interface	- The device interface.
//! @param [in] u8 function_selector            - The model number of the device.
//! @param [in,out] float *bias_vector		    - Pointer to array containing input bias vector on write commands or
//! to contain output bias vector on read commands.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
//! @retval MIP_INTERFACE_OK     The command was successful.\n
...
u16 mip_3dm_cmd_capture_gyro_bias(mip_interface* device_interface, u16 duration_ms, float* bias_vector) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_CAPTURE_GYRO_BIAS, (u8*)&command_data, sizeof(u16),
      &response_data, &response_data_size, MIP_INTERFACE_GYRO_BIAS_COMMAND_RESPONSE_TIMEOUT_MS);
...
u16 mip_3dm_cmd_hard_iron(mip_interface* device_interface, u8 function_selector, float* vector) {
  ...
  if (function_selector == MIP_FUNCTION_SELECTOR_WRITE) {
    ...
    // Byteswap if enabled
    if (MIP_SDK_CONFIG_BYTESWAP) {
      for (i = 0; i < 3; i++) byteswap_inplace(&vector_pointer[i], sizeof(float));
    }
  }

  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_HARD_IRON_VECTOR, command_data, sizeof(u8) + 3 * sizeof(float),
      &response_data, &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_3DM_REPLY_HARD_IRON_VECTOR) &&
      ...
      // Byteswap the baudrate if enabled
      if (MIP_SDK_CONFIG_BYTESWAP) {
        for (i = 0; i < 3; i++) byteswap_inplace(&vector[i], sizeof(float));
      }
    } else
      return_code = MIP_INTERFACE_ERROR;
  }
...
u16 mip_3dm_cmd_soft_iron(mip_interface* device_interface, u8 function_selector, float* matrix) {
  ...
  if (function_selector == MIP_FUNCTION_SELECTOR_WRITE) {
    ...
    // Byteswap if enabled
    if (MIP_SDK_CONFIG_BYTESWAP) {
      for (i = 0; i < 9; i++) byteswap_inplace(&matrix_pointer[i], sizeof(float));
    }
  }

  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_SOFT_IRON_MATRIX, command_data, sizeof(u8) + 9 * sizeof(float),
      &response_data, &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_3DM_REPLY_SOFT_IRON_MATRIX) &&
      ...
      // Byteswap the baudrate if enabled
      if (MIP_SDK_CONFIG_BYTESWAP) {
        for (i = 0; i < 9; i++) byteswap_inplace(&matrix[i], sizeof(float));
      }
    } else
      return_code = MIP_INTERFACE_ERROR;
  }
...
//
//! @fn
//! u16 mip_3dm_sensor2vehicle_tranformation(mip_interface *device_interface, u8 function_selector, float
//! euler_angles[3])
//
//! @section DESCRIPTION
...
u16 mip_3dm_sensor2vehicle_tranformation(mip_interface* device_interface, u8 function_selector, float euler_angles[3]) {
  ...
  if (function_selector == MIP_FUNCTION_SELECTOR_WRITE) {
    ...
    // Byteswap if enabled
    if (MIP_SDK_CONFIG_BYTESWAP) {
      for (i = 0; i < 3; i++) byteswap_inplace(&angle_pointer[i], sizeof(float));
    }
  }

  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CDM_SENSOR2VEHICLE_TRANSFORMATION, command_data,
...
//
//! @fn
//! u16 mip_3dm_cmd_complementary_filter_settings(mip_interface *device_interface, u8 function_selector,
...
u16 mip_3dm_cmd_complementary_filter_settings(mip_interface* device_interface, u8 function_selector,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_COMPLEMENTARY_FILTER, command_data,
...
u16 mip_3dm_cmd_rtcm_23_message(mip_interface* device_interface, u8* raw_data, u16 num_bytes) {
  ...
  while (curr_byte < num_bytes) {
    ...
    if (bytes_to_send > MIP_MAX_PAYLOAD_DATA_SIZE) bytes_to_send = MIP_MAX_PAYLOAD_DATA_SIZE;

    // Send the data
    if (mip_interface_send_command(device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_RAW_RTCM_2_3_MESSAGE,
                                   &raw_data[curr_byte], bytes_to_send, 1,
                                   MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS) != MIP_INTERFACE_OK) {
      return MIP_INTERFACE_ERROR;
    } else {
