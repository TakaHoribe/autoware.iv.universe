...
u16 ring_buffer_init_malloc(ring_buffer* buffer, u32 max_entries, u32 entry_size) {
  ...
  if (buffer->entries == 0) return RING_BUFFER_MEMORY_ERROR;
...
u32 ring_buffer_count(ring_buffer* buffer) {
  ...
  if (buffer->state != RING_BUFFER_INITIALIZED) return 0;
...
u16 ring_buffer_flush(ring_buffer* buffer) {
  ...
  // Check that the buffer was initialized
  if (buffer->state != RING_BUFFER_INITIALIZED) return RING_BUFFER_ERROR;
...
u16 ring_buffer_write(ring_buffer* buffer, u8* entry, u32 num_bytes) {
  ...
  if (num_bytes == 0) return RING_BUFFER_OK;

  // Check entry size
  if (num_bytes > buffer->entry_size) return RING_BUFFER_MEMORY_ERROR;
  ...
  // Write the entry
  for (i = 0; i < num_bytes; i++) buffer->entries[entry_pos * buffer->entry_size + i] = entry[i];
...
u16 ring_buffer_write_multi(ring_buffer* buffer, u8* entry_buff, u32 num_entries, u32* num_written) {
  ...
  *num_written = 0;
  ...
  if (num_entries == 0) return RING_BUFFER_OK;
...
u16 ring_buffer_read(ring_buffer* buffer, u8* entry, u32 max_bytes) {
  ...
  if (buffer->current_count == 0) return RING_BUFFER_EMPTY;

  // Copy the entry
  for (i = 0; i < buffer->entry_size; i++) entry[i] = buffer->entries[buffer->position * buffer->entry_size + i];
...
u16 ring_buffer_lookahead_read(ring_buffer* buffer, u32 offset, u8* entry, u32 max_bytes) {
  ...
  if (max_bytes < buffer->entry_size) return RING_BUFFER_MEMORY_ERROR;

  // Check that there are the expected number of entries (that count > offset)
  if (buffer->current_count <= offset) return RING_BUFFER_ERROR;
  ...
  // Copy the entry
  for (i = 0; i < buffer->entry_size; i++) entry[i] = buffer->entries[local_position * buffer->entry_size + i];
...
u16 ring_buffer_read_multi(ring_buffer* buffer, u8* entry_buff, u32 entry_buff_size, u32 num_requested, u32* num_read) {
  ...
  *num_read = 0;
  ...
  if (entry_buff_size < num_requested * buffer->entry_size) return RING_BUFFER_MEMORY_ERROR;
...
u16 ring_buffer_lookahead_read_multi(ring_buffer* buffer, u32 offset, u8* entry_buff, u32 entry_buff_size,
                                     u32 num_requested, u32* num_read) {
  ...
  *num_read = 0;
  ...
  // Check that there are the expected number of entries (that count > offset)
  if (buffer->current_count < offset + num_requested) return RING_BUFFER_ERROR;
...
u32 ring_buffer_copy(ring_buffer* to, ring_buffer* from, u32 num_entries) {
  ...
  if (from->entry_size != to->entry_size) return 0;

  // Loop through num_entries, copying each
  for (i = 0; i < num_entries; i++) {
    // Check if from buffer is empty
    if (from->current_count == 0) return num_copied;

    // Check if to buffer is full
    if (to->current_count >= to->max_entries) return num_copied;
...
u32 ring_buffer_lookahead_copy(ring_buffer* to, u32 offset, ring_buffer* from, u32 num_entries) {
  ...
  if (to->state != RING_BUFFER_INITIALIZED) return 0;

  // Check if the 'from' ring buffer is initialized
  if (from->state != RING_BUFFER_INITIALIZED) return 0;

  // Check that the entry size is the same
  if (from->entry_size != to->entry_size) return 0;

  // Check the offset and number of requested entries
  if (num_entries + offset > ring_buffer_count(from)) return 0;

  // Loop through num_entries, copying each
  for (i = 0; i < num_entries; i++) {
    // Check if from buffer is empty
    if (from->current_count == 0) return i;

    // Check if to buffer is full
    if (to->current_count >= to->max_entries) return i;
...
u16 ring_buffer_malloc_free(ring_buffer* buffer) {
  ...
  if (buffer->type != RING_BUFFER_MALLOC_TYPE) return RING_BUFFER_ERROR;
...
u8* ring_buffer_get_first_element_ptr(ring_buffer* buffer) {
  ...
  // Check for an empty buffer
  if (buffer->current_count == 0) return 0;
...
u8* ring_buffer_get_available_element_ptr(ring_buffer* buffer) {
  ...
  if (buffer->current_count >= buffer->max_entries) return 0;
...
u16 ring_buffer_increment_count(ring_buffer* buffer) {
  ...
  if (buffer->current_count >= buffer->max_entries) return RING_BUFFER_FULL;
