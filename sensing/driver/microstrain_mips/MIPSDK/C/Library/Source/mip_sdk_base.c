...
u16 mip_base_cmd_ping(mip_interface* device_interface) {
  return mip_interface_send_command(device_interface, MIP_BASE_COMMAND_DESC_SET, MIP_CMD_DESC_BASE_PING, NULL, 0, 1,
...
u16 mip_base_cmd_idle(mip_interface* device_interface) {
  return mip_interface_send_command(device_interface, MIP_BASE_COMMAND_DESC_SET, MIP_CMD_DESC_BASE_SET_TO_IDLE, NULL, 0,
                                    1, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
}
...
u16 mip_base_cmd_get_device_info(mip_interface* device_interface, base_device_info_field* device_info) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_BASE_COMMAND_DESC_SET, MIP_CMD_DESC_BASE_GET_DEVICE_INFO, NULL, 0, &response_data,
...
//
//! @fn
//! u16 mip_base_cmd_get_device_supported_descriptors(mip_interface *device_interface, u8 *response_buffer, u16
//! *response_size)
//
//! @section DESCRIPTION
...
//! @param [in, out] u16 *response_size         - On entry, the size of the buffer; On exit, the size of the data
//! returned.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_base_cmd_get_device_supported_descriptors(mip_interface* device_interface, u8* response_buffer,
                                                  u16* response_size) {
  u8* response_data;
  u16 response_data_size;
  u16 return_code;
  ...
  *response_size = 0;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_BASE_COMMAND_DESC_SET, MIP_CMD_DESC_BASE_GET_DEVICE_DESCRIPTORS, NULL, 0, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
    ...
    if ((field_header_ptr->size - sizeof(mip_field_header)) <= user_buffer_size) {
      ...
      *response_size = field_header_ptr->size - sizeof(mip_field_header);
      ...
      if (MIP_SDK_CONFIG_BYTESWAP) {
        ...
        for (i = 0; i < *response_size / 2; i++) byteswap_inplace(&short_ptr[i], sizeof(u16));
      }
    } else {
      *response_size = 0;
      return_code = MIP_INTERFACE_ERROR;
...
u16 mip_base_cmd_built_in_test(mip_interface* device_interface, u32* bit_result) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_BASE_COMMAND_DESC_SET, MIP_CMD_DESC_BASE_BUILT_IN_TEST, NULL, 0, &response_data,
...
u16 mip_base_cmd_resume(mip_interface* device_interface) {
  return mip_interface_send_command(device_interface, MIP_BASE_COMMAND_DESC_SET, MIP_CMD_DESC_BASE_RESUME, NULL, 0, 1,
                                    MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
}
...
u16 mip_base_cmd_reset_device(mip_interface* device_interface) {
  return mip_interface_send_command(device_interface, MIP_BASE_COMMAND_DESC_SET, MIP_CMD_DESC_BASE_SOFT_RESET, NULL, 0,
                                    1, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
}
