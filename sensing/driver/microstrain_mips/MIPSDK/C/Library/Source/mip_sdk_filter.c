...
void mip_filter_attitude_quaternion_byteswap(mip_filter_attitude_quaternion* attitude_quaternion) {
  ...
  for (i = 0; i < 4; i++) byteswap_inplace(&attitude_quaternion->q[i], sizeof(float));
...
void mip_filter_attitude_dcm_byteswap(mip_filter_attitude_dcm* attitude_dcm) {
  ...
  for (i = 0; i < 3; i++) {
    for (j = 0; j < 3; j++) byteswap_inplace(&attitude_dcm->dcm[i][j], sizeof(float));
  }
...
//
//! @fn
//! void mip_filter_euler_attitude_uncertainty_byteswap(mip_filter_euler_attitude_uncertainty
//! *euler_attitude_uncertainty)
//
//! @section DESCRIPTION
...
//
//! @fn
//! void mip_filter_quaternion_attitude_uncertainty_byteswap(mip_filter_quaternion_attitude_uncertainty
//! *quaternion_attitude_uncertainty)
//
//! @section DESCRIPTION
...
//! @section DETAILS
//!
//! @param [in] mip_filter_quaternion_attitude_uncertainty *quaternion_attitude_uncertainty - The structure to be
//! byteswapped.
//
//! @section NOTES
//!
...
void mip_filter_quaternion_attitude_uncertainty_byteswap(
    mip_filter_quaternion_attitude_uncertainty* quaternion_attitude_uncertainty) {
...
//
//! @fn
//! void mip_filter_external_heading_update_with_time_byteswap(mip_filter_external_heading_with_time_command
//! *external_heading_command);
//
//! @section DESCRIPTION
...
void mip_filter_external_heading_update_with_time_byteswap(
    mip_filter_external_heading_with_time_command* external_heading_command) {
...
//
//! @fn
//! void mip_filter_accel_scale_factor_uncertainty_byteswap(mip_filter_accel_scale_factor_uncertainty_mip_field
//! *accel_scale_factor_uncertainty)
//
//! @section DESCRIPTION
...
//! @section DETAILS
//!
//! @param [in] mip_filter_accel_scale_factor_uncertainty_mip_field *accel_scale_factor_uncertainty - The structure to
//! be byteswapped.
//
//! @section NOTES
//!
...
void mip_filter_accel_scale_factor_uncertainty_byteswap(
    mip_filter_accel_scale_factor_uncertainty_mip_field* accel_scale_factor_uncertainty) {
...
//
//! @fn
//! void mip_filter_gyro_scale_factor_uncertainty_byteswap(mip_filter_gyro_scale_factor_uncertainty_mip_field
//! *gyro_scale_factor_uncertainty)
//
//! @section DESCRIPTION
...
//! @section DETAILS
//!
//! @param [in] mip_filter_gyro_scale_factor_uncertainty_mip_field *gyro_scale_factor_uncertainty - The structure to be
//! byteswapped.
//
//! @section NOTES
//!
...
void mip_filter_gyro_scale_factor_uncertainty_byteswap(
    mip_filter_gyro_scale_factor_uncertainty_mip_field* gyro_scale_factor_uncertainty) {
...
//
//! @fn
//! void mip_filter_antenna_offset_correction_byteswap(mip_filter_antenna_offset_correction_mip_field
//! *antenna_offset_correction)
//
//! @section DESCRIPTION
...
//! @section DETAILS
//!
//! @param [in] mip_filter_antenna_offset_correction_mip_field *antenna_offset_correction - The structure to be
//! byteswapped.
//
//! @section NOTES
//!
...
void mip_filter_antenna_offset_correction_byteswap(
    mip_filter_antenna_offset_correction_mip_field* antenna_offset_correction) {
...
//! @fn
//! void
//! mip_filter_antenna_offset_correction_uncertainty_byteswap(mip_filter_antenna_offset_correction_uncertainty_mip_field
//! *antenna_offset_correction_uncertainty)
//
//! @section DESCRIPTION
...
//! @section DETAILS
//!
//! @param [in] mip_filter_antenna_offset_correction_uncertainty_mip_field *antenna_offset_correction_uncertainty - The
//! structure to be byteswapped.
//
//! @section NOTES
//!
//! None
...
void mip_filter_antenna_offset_correction_uncertainty_byteswap(
    mip_filter_antenna_offset_correction_uncertainty_mip_field* antenna_offset_correction_uncertainty) {
...
u16 mip_filter_reset_filter(mip_interface* device_interface) {
  return mip_interface_send_command(device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_RESET_FILTER, NULL, 0, 1,
...
u16 mip_filter_set_init_heading(mip_interface* device_interface, float heading) {
  ...
  return mip_interface_send_command(device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_SET_INITIAL_HEADING,
                                    (u8*)&local_heading, sizeof(float), 1,
                                    MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
}
...
//
//! @section DESCRIPTION
//! Initialize the Kalman Filter from the AHRS algorithm output, taking into account the magnetic declination angle
//! povided.
//
//! @section DETAILS
//!
...
u16 mip_filter_set_init_attitude_from_ahrs(mip_interface* device_interface, float declination) {
  ...
  return mip_interface_send_command(device_interface, MIP_FILTER_COMMAND_SET,
                                    MIP_FILTER_CMD_SET_INITIAL_HEADING_FROM_AHRS, (u8*)&local_declination,
                                    sizeof(float), 1, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
}
...
//! @param [in,out] u8 *dynamics_mode                   - The vehicle dynamics mode. (used to set or get depending on \c
//! function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_filter_vehicle_dynamics_mode(mip_interface* device_interface, u8 function_selector, u8* dynamics_mode) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_VEHICLE_DYNAMICS_MODE, command_data, 2, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_filter_sensor2vehicle_tranformation(mip_interface *device_interface, u8 function_selector, float
//! euler_angles[3])
//
//! @section DESCRIPTION
...
//! @param [in] mip_interface *device_interface        - The device interface.
//! @param [in] u8 function_selector                   - Selects which function to perform.
//! @param [in,out] float euler_angles[3]              - The sensor to vehicle transformation expressed as Euler
//! angles\n
//!                                                      in radians. (used to set or get depending on \c
//!                                                      function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_filter_sensor2vehicle_tranformation(mip_interface* device_interface, u8 function_selector,
  ...
  if (function_selector == MIP_FUNCTION_SELECTOR_WRITE) {
    ...
    // Byteswap the angles if enabled
    if (MIP_SDK_CONFIG_BYTESWAP) {
      for (i = 0; i < 3; i++) byteswap_inplace(&float_ptr[i], sizeof(float));
    }
  }

  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_SENSOR2VEHICLE_TRANSFORMATION, command_data,
      sizeof(u8) + sizeof(float) * 3, &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_FILTER_REPLY_SENSOR2VEHICLE_TRANSFORMATION) &&
      ...
      // Byteswap the angles if enabled
      if (MIP_SDK_CONFIG_BYTESWAP) {
        for (i = 0; i < 3; i++) byteswap_inplace(&euler_angles[i], sizeof(float));
      }
    } else
      return_code = MIP_INTERFACE_ERROR;
  }
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] float offset                - The sensor to vehicle frame offset in meters. (used to set or get
//! depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
//! @retval MIP_INTERFACE_OK     The command was successful.\n
...
u16 mip_filter_sensor2vehicle_offset(mip_interface* device_interface, u8 function_selector, float offset[3]) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_SENSOR2VEHICLE_OFFSET, command_data,
      sizeof(u8) + sizeof(float) * 3, &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_FILTER_REPLY_SENSOR2VEHICLE_OFFSET) &&
      ...
      // Byteswap the angles if enabled
      if (MIP_SDK_CONFIG_BYTESWAP) {
        for (i = 0; i < 3; i++) byteswap_inplace(&offset[i], sizeof(float));
      }
    } else
      return_code = MIP_INTERFACE_ERROR;
  }
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] float offset                - The offset of the antenna with-respect-to the sensor in meters. (used
//! to set or get depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
//! @retval MIP_INTERFACE_OK     The command was successful.\n
...
u16 mip_filter_antenna_offset(mip_interface* device_interface, u8 function_selector, float offset[3]) {
  ...
  return_code =
      mip_interface_send_command_with_response(device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_ANTENNA_OFFSET,
                                               command_data, sizeof(u8) + sizeof(float) * 3, &response_data,
                                               &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] u8 *gps_source              - The source of GPS updates to the filter. (used to set or get depending
//! on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
//! @retval MIP_INTERFACE_OK     The command was successful.\n
...
u16 mip_filter_gps_source(mip_interface* device_interface, u8 function_selector, u8* gps_source) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_GPS_SOURCE_CONTROL, command_data, 2, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
u16 mip_filter_external_gps_update(mip_interface* device_interface, mip_filter_external_gps_update_command* command) {
  ...
  return mip_interface_send_command(device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_EXTERNAL_GPS_UPDATE,
                                    (u8*)&local_command, sizeof(mip_filter_external_gps_update_command), 1,
                                    MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
}
...
//
//! @fn
//! u16 mip_filter_external_heading_update(mip_interface *device_interface, mip_filter_external_heading_update_command
//! *command)
//
//! @section DESCRIPTION
...
u16 mip_filter_external_heading_update(mip_interface* device_interface,
  ...
  return mip_interface_send_command(device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_EXTERNAL_HEADING_UPDATE,
                                    (u8*)&local_command, sizeof(mip_filter_external_heading_update_command), 1,
                                    MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
}
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] u8 *heading_source          - The source of heading updates to the filter. (used to set or get
//! depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_filter_heading_source(mip_interface* device_interface, u8 function_selector, u8* heading_source) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_HEADING_UPDATE_CONTROL, command_data, 2, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] u8 *enable                  - The auto-initialization setting. (used to set or get depending on \c
//! function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_filter_auto_initialization(mip_interface* device_interface, u8 function_selector, u8* enable) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_AUTOINIT_CONTROL, command_data, 2, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] float noise_1sigma[3]       - The accelerometer 1-sigma process/measurement noise values in m/(s^2).
//! (used to set or get depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
//! @retval MIP_INTERFACE_OK     The command was successful.\n
...
u16 mip_filter_accel_noise(mip_interface* device_interface, u8 function_selector, float noise_1sigma[3]) {
  ...
  return_code =
      mip_interface_send_command_with_response(device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_ACCEL_NOISE,
                                               command_data, sizeof(u8) + sizeof(float) * 3, &response_data,
                                               &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  // Copy the data to the provided buffer on success if present
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_FILTER_REPLY_ACCEL_NOISE) &&
      ...
      // Byteswap the angles if enabled
      if (MIP_SDK_CONFIG_BYTESWAP) {
        for (i = 0; i < 3; i++) byteswap_inplace(&noise_1sigma[i], sizeof(float));
      }
    } else
      return_code = MIP_INTERFACE_ERROR;
  }
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] float noise_1sigma[3]       - The gyroscope 1-sigma process noise values in rad/sec. (used to set or
//! get depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
//! @retval MIP_INTERFACE_OK     The command was successful.\n
...
u16 mip_filter_gyro_noise(mip_interface* device_interface, u8 function_selector, float noise_1sigma[3]) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_GYRO_NOISE, command_data, sizeof(u8) + sizeof(float) * 3,
      &response_data, &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_filter_gyro_bias_model(mip_interface *device_interface, u8 function_selector, float bias_beta[3], float
//! bias_noise_1sigma[3])
//
//! @section DESCRIPTION
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] float bias_beta[3]          - The gyroscope bias model time constants in 1/sec. (used to set or get
//! depending on \c function_selector)
//! @param [in,out] float bias_noise_1sigma[3]  - The gyroscope bias model 1-sigma white noise values in rad/sec. (used
//! to set or get depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
//! sensor frame in units of 1/sec.  The gyro Gauss-Markov bias model 1-sigma white noise values are ordered [x, y, z]
//! in the\n sensor frame in units of rad/sec.  These values are used as process noise to the gyro bias model and allow
//! the user to tune\n the behavior of the Kalman filter for their particular application.
//!
//
/////////////////////////////////////////////////////////////////////////////

u16 mip_filter_gyro_bias_model(mip_interface* device_interface, u8 function_selector, float bias_beta[3],
  ...
  if (function_selector == MIP_FUNCTION_SELECTOR_WRITE) {
    ...
    // Byteswap the angles if enabled
    if (MIP_SDK_CONFIG_BYTESWAP) {
      for (i = 0; i < 6; i++) byteswap_inplace(&float_ptr[i], sizeof(float));
    }
  }

  return_code =
      mip_interface_send_command_with_response(device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_GYRO_BIAS_MODEL,
                                               command_data, sizeof(u8) + sizeof(float) * 6, &response_data,
                                               &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_filter_accel_bias_model(mip_interface *device_interface, u8 function_selector, float bias_beta[3], float
//! bias_noise_1sigma[3])
//
//! @section DESCRIPTION
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] float bias_beta[3]          - The accelerometer bias model time constants in 1/sec. (used to set or
//! get depending on \c function_selector)
//! @param [in,out] float bias_noise_1sigma[3]  - The accelerometer bias model 1-sigma white noise values in rad/sec.
//! (used to set or get depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
//! sensor frame in units of 1/sec.  The accel Gauss-Markov bias model 1-sigma white noise values are ordered [x, y, z]
//! in the\n sensor frame in units of rad/sec.  These values are used as process noise to the accel bias model and allow
//! the user to tune\n the behavior of the Kalman filter for their particular application.
//!
//
/////////////////////////////////////////////////////////////////////////////

u16 mip_filter_accel_bias_model(mip_interface* device_interface, u8 function_selector, float bias_beta[3],
                                float bias_noise_1sigma[3]) {
  u8 i;
  u8* response_data;
  u16 response_data_size;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_ACCEL_BIAS_MODEL, command_data,
      sizeof(u8) + sizeof(float) * 6, &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_filter_zero_velocity_update_control(mip_interface *device_interface, u8 function_selector,
//! mip_filter_zero_update_command_velocity *zero_velocity_control)
//
//! @section DESCRIPTION
...
//! @param [in,out] float threshold             - The GPS velocity magnitude below which the device is considered to be
//! stationary.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_filter_zero_velocity_update_control(mip_interface* device_interface, u8 function_selector,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_ZERO_VELOCITY_UPDATE, command_data,
      sizeof(u8) + sizeof(mip_filter_zero_update_command), &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_filter_zero_angular_rate_update_control(mip_interface *device_interface, u8 function_selector,
...
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] u8 enable                   - Writes/Reads enable bit state.
//! @param [in,out] float threshold             - The gyro magnitude below which the device is considered to be
//! stationary.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_filter_zero_angular_rate_update_control(mip_interface* device_interface, u8 function_selector,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_ANGULAR_RATE_ZERO_UPDATE, command_data,
      sizeof(u8) + sizeof(mip_filter_zero_update_command), &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in] u8 tare_bitfield                - Tare bitfield (where 0x01 bit represents the roll axis, 0x02
//! represents the pitch axis, 0x04 represents the yaw axis).
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
//! @retval MIP_INTERFACE_OK     The command was successful.\n
...
u16 mip_filter_tare_orientation(mip_interface* device_interface, u8 function_selector, u8 tare_bitfield) {
  ...
  return_code = mip_interface_send_command(device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_TARE_ORIENTATION,
                                           command_data, 2, 1, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
u16 mip_filter_commanded_zero_velocity_update(mip_interface* device_interface) {
  ...
  return_code = mip_interface_send_command(device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_COMMANDED_ZUPT,
...
u16 mip_filter_commanded_zero_angular_rate_update(mip_interface* device_interface) {
  ...
  return_code = mip_interface_send_command(device_interface, MIP_FILTER_COMMAND_SET,
                                           MIP_FILTER_CMD_COMMANDED_ZERO_ANGULAR_RATE_UPDATE, NULL, 0, 1,
                                           MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_filter_external_heading_update_with_time(mip_interface *device_interface,
...
//! @param [in] mip_interface *device_interface        - The device interface.
//! @param [in] u8 function_selector                   - Selects which function to perform.
//! @param [in,out] mip_filter_external_heading_with_time_command *heading_with_time_command - The external heading
//! update with timestamp command structure. (used to set or get depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
//! @retval MIP_INTERFACE_OK     The command was successful.\n
...
u16 mip_filter_external_heading_update_with_time(
    mip_interface* device_interface, mip_filter_external_heading_with_time_command* heading_with_time_command) {
  u16 return_code;
  ...
  return_code = mip_interface_send_command(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_EXTERNAL_HEADING_TIMESTAMP, command_data,
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] float noise_1sigma[3]       - The magnetometer 1-sigma measurement noise values in Gauss. (used to
//! set or get depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
//! @retval MIP_INTERFACE_OK     The command was successful.\n
...
u16 mip_filter_mag_noise(mip_interface* device_interface, u8 function_selector, float noise_1sigma[3]) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_MAG_NOISE, command_data, sizeof(u8) + sizeof(float) * 3,
      &response_data, &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_filter_reference_position(mip_interface *device_interface,  u8 function_selector, u8 *reference_enable,
...
//! @param [in] mip_interface *device_interface  - The device interface.
//! @param [in] u8 function_selector             - Selects which function to perform.
//! @param [in,out] u8 *reference_enable         - Pointer to the enable reference position flag. (used to set or get
//! depending on \c function_selector)
//! @param [in,out] double reference_position[3] - The reference position [Latitude (deg), Longitude (deg), and Height
//! (m)]. (used to set or get depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
//! @retval MIP_INTERFACE_OK     The command was successful.\n
...
u16 mip_filter_reference_position(mip_interface* device_interface, u8 function_selector, u8* reference_enable,
  ...
  if (function_selector == MIP_FUNCTION_SELECTOR_WRITE) {
    ...
    // Byteswap the angles if enabled
    if (MIP_SDK_CONFIG_BYTESWAP) {
      for (i = 0; i < 3; i++) byteswap_inplace(&double_ptr[i], sizeof(double));
    }
  }

  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_REFERENCE_POSITION, command_data,
      sizeof(u8) + sizeof(u8) + sizeof(double) * 3, &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_FILTER_REPLY_REFERENCE_POSITION) &&
      ...
      if (MIP_SDK_CONFIG_BYTESWAP) {
        for (i = 0; i < 3; i++) byteswap_inplace(&reference_position[i], sizeof(double));
      }
    } else
      return_code = MIP_INTERFACE_ERROR;
  }
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] u16 *estimation_control     - The estimation control value. (used to set or get depending on \c
//! function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_filter_estimation_control(mip_interface* device_interface, u8 function_selector, u16* estimation_control) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_ESTIMATION_CONTROL, command_data, 3, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] u16 *measurement_enable     - The measurement enable value. (used to set or get depending on \c
//! function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_filter_enable_measurement(mip_interface* device_interface, u8 function_selector, u16* measurement_enable) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_ENABLE_MEASUREMENT, command_data, 3, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] u8 *declination_source      - The source of declination provided to the filter. (used to set or get
//! depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
...
u16 mip_filter_declination_source(mip_interface* device_interface, u8 function_selector, u8* declination_source) {
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_DECLINATION_SOURCE, command_data, 2, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_filter_accel_magnitude_error_adaptive_measurement(mip_interface *device_interface, u8 function_selector,
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] mip_filter_accel_magnitude_error_adaptive_measurement_command *params - The accelerometer magnitude
//! error adaptive measurement settings.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
//! @retval MIP_INTERFACE_OK     The command was successful.\n
...
u16 mip_filter_accel_magnitude_error_adaptive_measurement(
    mip_interface* device_interface, u8 function_selector,
    mip_filter_accel_magnitude_error_adaptive_measurement_command* params) {
  u8 i;
  u8* response_data;
  u16 response_data_size;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_ACCEL_MAGNITUDE_ERROR_ADAPTIVE_MEASUREMENT_CONTROL,
      command_data, sizeof(u8) + sizeof(mip_filter_accel_magnitude_error_adaptive_measurement_command), &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_FILTER_REPLY_ACCEL_MAGNITUDE_ERROR_ADAPTIVE_MEASUREMENT_CONTROL) &&
        (field_header_ptr->size >=
         sizeof(mip_field_header) + sizeof(mip_filter_accel_magnitude_error_adaptive_measurement_command))) {
...
//
//! @fn
//! u16 mip_filter_mag_magnitude_error_adaptive_measurement(mip_interface *device_interface, u8 function_selector,
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] mip_filter_magnetometer_magnitude_error_adaptive_measurement_command *params - The magnetometer
//! magnitude error adaptive measurement settings.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
//! @retval MIP_INTERFACE_OK     The command was successful.\n
...
u16 mip_filter_mag_magnitude_error_adaptive_measurement(
    mip_interface* device_interface, u8 function_selector,
    mip_filter_magnetometer_magnitude_error_adaptive_measurement_command* params) {
  u8 i;
  u8* response_data;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_MAG_MAGNITUDE_ERROR_ADAPTIVE_MEASUREMENT_CONTROL,
      command_data, sizeof(u8) + sizeof(mip_filter_magnetometer_magnitude_error_adaptive_measurement_command),
      &response_data, &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_FILTER_REPLY_MAG_MAGNITUDE_ERROR_ADAPTIVE_MEASUREMENT_CONTROL) &&
        (field_header_ptr->size >=
         sizeof(mip_field_header) + sizeof(mip_filter_magnetometer_magnitude_error_adaptive_measurement_command))) {
      params->enable = *(response_data + sizeof(mip_field_header));
...
//
//! @fn
//! u16 mip_filter_mag_dip_angle_error_adaptive_measurement(mip_interface *device_interface, u8 function_selector,
...
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u8 function_selector            - Selects which function to perform.
//! @param [in,out] mip_filter_magnetometer_dip_angle_error_adaptive_measurement_command *params - The magnetometer dip
//! angle error adaptive measurement settings.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format or the\n
//!                              the device communications failed.\n
//! @retval MIP_INTERFACE_OK     The command was successful.\n
...
u16 mip_filter_mag_dip_angle_error_adaptive_measurement(
    mip_interface* device_interface, u8 function_selector,
    mip_filter_magnetometer_dip_angle_error_adaptive_measurement_command* params) {
  u8 i;
  u8* response_data;
  ...
  if (function_selector == MIP_FUNCTION_SELECTOR_WRITE) {
    ...
    // Byteswap the angles if enabled
    if (MIP_SDK_CONFIG_BYTESWAP) {
      for (i = 0; i < 4; i++) byteswap_inplace(&float_ptr[i], sizeof(float));
    }
  }

  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_FILTER_COMMAND_SET, MIP_FILTER_CMD_MAG_DIP_ANGLE_ERROR_ADAPTIVE_MEASUREMENT_CONTROL,
      command_data, sizeof(u8) + sizeof(mip_filter_magnetometer_dip_angle_error_adaptive_measurement_command),
      &response_data, &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_FILTER_REPLY_MAG_DIP_ANGLE_ERROR_ADAPTIVE_MEASUREMENT_CONTROL) &&
        (field_header_ptr->size >=
         sizeof(mip_field_header) + sizeof(mip_filter_magnetometer_dip_angle_error_adaptive_measurement_command))) {
      params->enable = *(response_data + sizeof(mip_field_header));
